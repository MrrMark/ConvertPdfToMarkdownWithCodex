...
이 문서는 Docling을 runtime backend로 붙이지 않고, Docling에서 참고할 만한 설계 아이디어를 `pdf2md` 네이티브 기능으로 재구성하기 위한 active 개발 명세다.
...
Status: Implemented in Q118. Active follow-up work continues in Q119-Q125.
...
- `page_layout_rag.jsonl` 또는 `layout_rag.jsonl` 후보 schema
- `report.summary` layout metric
- multi-column/header-footer/caption fixture

### 검증

- `python -m pytest tests/test_text_layout.py tests/test_header_footer.py tests/test_rag_text_blocks.py`
- layout sidecar schema contract test
- latest NVMe front matter and figure/table slice smoke
...
- `figure_ocr_evidence_rag.jsonl` 신규 sidecar 또는 기존 `figures_rag.jsonl` 확장
- report summary counters
- diagram/image-only-table fixture

### 검증

- `python -m pytest tests/test_rag_figures.py tests/test_images.py tests/test_ocr.py`
...
- cross-spec benchmark compatibility report 후보
