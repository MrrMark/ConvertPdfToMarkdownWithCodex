...
CLI와 GUI headless runner의 산출물 동등성은 별도 parity script 또는 optional release gate로 확인한다.
...
9. `기본 모드(원본 유지)`, `RAG 등록용(최적화)`, `기술 스펙 RAG`, `기술 스펙 Visual RAG`, `이미지 업로드 불가 RAG 대응`, `Optimize Options(유저 선택)` preset 변경 시 세부 옵션 잠금/해제가 맞는지 확인한다.
