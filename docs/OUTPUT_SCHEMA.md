...
- TCG/SPDM/Caliptra adapters may also emit source-text-derived review candidates when no table row is available. These records set `candidate_status="review_only"`, `review_required=true`, `candidate_kind`, `review_reasons[]`, and `candidate_evidence.source_text_span`; they keep `text` as the original source block and must not be treated as confirmed table-derived domain units.
