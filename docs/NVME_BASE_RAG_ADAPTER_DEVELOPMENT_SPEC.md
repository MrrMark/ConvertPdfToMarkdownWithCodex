# NVMe Base and Command Set RAG Adapter Development Spec

작성일: 2026-06-14

## 문서 목적

이 문서는 PDF2MD repo에서 NVMe Base Specification과 NVM Command Set Specification을
`technical_spec_rag` 용도로 변환할 때 필요한 개발 플랜과 구현 명세를 정의한다.
SSD Verification Agent의
...
- NVMe Base와 NVM Command Set은 공개 spec이지만 운영 contract는 Secure RAG 사용을 전제로 설계한다.
- NVMe-MI, transport, boot, ZNS, KV 등도 중요하지만 이 개발 track의 first-class 대상은 가장 자주 확인되는 NVMe Base와 NVM Command Set이다.
...
- NVMe Base/NVM Command Set taxonomy가 command/log/feature/register/status/data-structure 중심으로 아직 얕다.
...
- NVMe Base와 NVM Command Set real-spec benchmark gate가 부족하다.
...
- NVMe Base/NVM Command Set table row를 downstream traceability에 필요한 typed unit으로 더 정밀하게 분류한다.
...
- `Command Dword`, `Command DW`, `CDW`, `Dword`, `DW`
- `Pointer`, `Pointer Type`, `Command Pointer`, `Data Pointer`, `Metadata Pointer`
- `Command Scope`, `Command Type`, `Queue Type`, `Submission Queue Type`, `Command Set`
...
| `opcode` | `0x06` |
| `command_dword` | `CDW10` |
| `command_scope` | `admin`, `io` |
| `queue_type` | `admin`, `io` |
| `pointer_type` | `metadata`, `data` |
| `command_context` | `Read`, `Get Log Page` |
| `command_context_source` | `explicit_command_column`, `heading_path`, `previous_command_row` |
| `related_command_unit_id` | `tech-table-000001` |
...
| `status_code_group` | `generic`, `command_specific`, `media_data_integrity`, `path_related` |
| `error_class` | `success`, `invalid_address`, `invalid_request`, `data_integrity` |
| `retry_hint` | `not_applicable`, `correct_command`, `retry_allowed`, `do_not_retry` |
...
- NVM Command Set의 CDW/pointer/status row는 command heading이나 command opcode anchor가 명확할 때만 command relationship metadata를 갖는다.
...
- explicit requirement ID가 없는 NVMe Base/NVM Command Set 문단도 stable seed를 갖는다.
...
- NVMe Base/NVM Command Set에서 requirement 후보가 과추출되지 않도록 front matter, legal, introduction,
...
### P2M-NVME-7. NVMe Golden Slice Pack

목표:

- latest NVMe Base/NVM Command Set full PDF를 repo에 넣지 않고, sanitized/minimal fixture로 adapter quality를 잠근다.
...
### P2M-NVME-8. Latest NVMe Base and Command Set Benchmark

목표:

- real latest NVMe Base와 NVM Command Set conversion의 성능과 산출물 품질을 정량화한다.
...
- source URL, source_sha256, option matrix, summary counts, sanitized contract status, sanitized Command Set P2 query-eval summary만 포함한다.
- NVM Command Set benchmark는 `command_opcode`, `command_dword_field`, `command_pointer_field`, `status_code`
  대표 row를 대상으로 local query eval을 실행하고, query/result 원문 없이 aggregate metric만 기록한다.
- full precision mode와 fast smoke mode를 구분한다.
- `spec_document_type`으로 `base`와 `nvm_command_set`을 구분한다.
- `--fail-on-command-eval-error`는 NVM Command Set P2 query eval 실패를 non-zero exit로 승격한다.
...
- NVMe Base와 NVM Command Set이 같은 `domain_adapter=nvme` contract를 쓰되 benchmark report에서는 `spec_document_type`으로 구분된다.
...
5. P2M-NVME-7: NVMe golden slice pack.
6. P2M-NVME-8: latest NVMe Base/NVM Command Set benchmark.
