...
- Q110-Q125는 보수적 rollout으로 완료했다. Docling 설치 근거 없이 Docling runtime backend를 canonical 변환 경로에 승격하지 않는다.
- OCR backend는 `tesseract`, `tesseract-cli`, `rapidocr`, `ocrmac` adapter registry로 확장했지만 기본값은 계속 `tesseract`다.
- layout, table, chunk, OCR, figure, domain 개선은 `pdf2md` 네이티브 IR/sidecar/validator 계약으로 구현했다.
...
Current tool baseline after Q125:

- OCR은 `pypdfium2` page render 기반이며, conversion-time backend registry가 `tesseract`, `tesseract-cli`, `rapidocr`, `ocrmac`를 지원한다. optional backend import/runtime 실패는 structured warning/report로 남긴다.
...
| --- | --- | --- | --- | --- | --- |
| Multi OCR backend | engine option을 분리하고 optional dependency로 다룬다 | Q123에서 `tesseract`, `tesseract-cli`, `rapidocr`, `ocrmac` registry 구현 | scanned PDF/한글/저해상도 OCR 품질 비교 가능 | backend별 language/confidence 의미가 다름 | 구현 완료. 기본 backend는 `tesseract`, optional backend 실패는 warning/report |
| Region OCR | OCR을 page 전체가 아니라 image/table/figure region에 한정할 수 있다 | Q122에서 evidence sidecar 구현 | figure/table 내부 text evidence 보강 | bbox crop 오류가 원문을 오염시킬 수 있음 | report-only 유지. Markdown/text/table source 대체 금지 |
| Picture description | local VLM description을 optional enrichment로 분리한다 | Q113-Q124에서 local-only eval과 deterministic sidecar/review metadata 구현 | 이미지 업로드 불가 RAG에서 diagram 검색성 보완 가능 | 환각, 저작권/보안, 원문과 생성 텍스트 혼동 | sidecar-only/review-required 유지. 본문 삽입 금지 |
| Layout-aware table/figure adapter | table structure fast/accurate, layout model, unified document representation | Q114 comparison-only, Q118-Q121 native IR/layout sidecar 구현 | 복잡 표/figure bbox 비교 기준 확보 | current output과 섞으면 결정성/원문 보존 위험 | Docling merge/blend 금지. native sidecar와 sanitized comparison 유지 |
| Domain adapter registry | document representation과 metadata registry를 분리한다 | Q125에서 `DomainAdapterSpec` registry 구현 | cross-spec 재처리, SSD agent handoff, 고객 requirement adapter 안정화 | private customer schema를 core에 고정할 위험 | built-in registry만 유지. 외부 plugin loading은 기본 비활성 |
...
pdf2md/
  adapters/
    docling_adapter.py          # optional comparison-only import if a future Q opens it
  extractors/
    ocr_backends/
    base.py                     # OCR backend protocol and normalized result
    tesseract.py                # current behavior
    tesseract_cli.py            # system tesseract executable
    rapidocr.py                 # optional dependency
    ocrmac.py                   # optional dependency
  serializers/
    rag_domain_adapters.py      # DomainAdapterSpec registry
```

Implemented public options:

- `--ocr-backend tesseract|tesseract-cli|rapidocr|ocrmac`
- `--figure-region-ocr` default false
...
## Implemented Native Schema

Manifest/report options now include:

- `options.ocr_backend`
- `options.figure_region_ocr`
- `options.rag_generated_figure_descriptions`
- `options.figure_structure_extraction`
- `options.layout_comparison_mode`
- `options.domain_adapter`

Report summary fields now include:
...
Warning/reason families include:
...
- `missing_adapter_metadata` in the SSD-RAG validator for legacy domain-unit records
...
현재 시점에서는 남은 작업을 active backlog로 등록하지 않는다.
Q110/Q111에서 runtime probe와 OCR backend adapter contract를 완료했고, Q112에서 region OCR report-only prototype, Q113에서 local-only picture description evaluation pack, Q114에서 Docling layout adapter comparison mode를 완료했다.
Q118-Q125에서는 Docling-inspired 설계를 `pdf2md` native IR, layout sidecar, region OCR evidence, figure semantics, domain adapter registry로 이식했다.
추가 Docling-installed benchmark evidence가 확보되면 별도 Q 작업을 생성한다.
