...
Windows setup script 기본값은 최신 안정화 검증축인 `Python 3.14`와 `.venv314`다. `py -3.14` 또는 Python 3.14.x를 가리키는 `python` 명령이 없으면, 스크립트는 `winget`으로 `Python.Python.3.14` 설치를 먼저 시도한다. 회사 보안 정책으로 `winget` 또는 외부 설치가 막히면 에러 메시지에 python.org 수동 설치와 venv fallback 명령을 출력한다.

Python 3.11/3.12/3.13만 설치된 PC에서 새 Python 설치를 원하지 않으면 설치된 minor version과 가상환경 이름을 함께 명시한다.

```powershell
.\scripts\setup_windows_env.ps1 -PythonVersion 3.12 -VenvDir .venv312 -SkipWingetInstall
```
...
- Python 실행기 확인 (`py -3.14` 우선, 없으면 `python --version`이 Python 3.14.x인지 확인)
- Python 3.14 실행기가 없으면 `winget` 자동 설치 시도
- `.venv314` 가상환경 생성 또는 재사용
- 기존 가상환경 Python 버전 검증
- `pip` 업그레이드
- 프로젝트와 개발/검증 의존성 설치
...
최소 지원축인 Python 3.11로 별도 환경을 만들고 싶으면 PowerShell script를 직접 실행한다.

```powershell
.\scripts\setup_windows_env.ps1 -PythonVersion 3.11 -VenvDir .venv311 -SkipWingetInstall
```

기존 가상환경의 Python 버전이 요청한 버전과 다르다는 오류가 나오면 다른 `-VenvDir`를 쓰거나, 재생성을 명시한다.
...
Python 3.11 fallback을 명시해서 `.venv311`을 만들었다면:
