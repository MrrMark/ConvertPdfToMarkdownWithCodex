...
Q53에서 최소 desktop GUI wrapper가 추가됐지만, 실제 사용자 환경에서는 Tkinter 미설치, 잘못된 Python 버전, editable install 누락, entry point 경로 문제, output directory 권한 문제처럼 변환 엔진 이전 단계에서 실패할 수 있다. 이 실패가 일반 변환 실패처럼 보이면 비개발자가 원인을 파악하기 어렵다.

#### 구현 범위

- GUI 실행 전 runtime diagnostic 함수를 추가한다.
- 진단 대상은 Python 버전, Tkinter import 가능 여부, `pdf2md-gui`/module 실행 경로, input/output path 접근 가능성, output directory 생성 가능성으로 제한한다.
- diagnostic 결과는 GUI log와 시작 전 validation 메시지에서 조치 가능한 문장으로 표시한다.
- `python -m pdf2md.gui --help`는 계속 창을 띄우지 않고 종료해야 한다.
- CLI `pdf2md` 변환 경로, public conversion JSON schema, Markdown 산출물 계약은 변경하지 않는다.

#### 테스트/검증

- Tkinter import 실패를 monkeypatch로 재현하는 unit test.
- output path 권한/파일 충돌 validation unit test.
- `python -m pdf2md.gui --help` smoke test 유지.
- `env PYTHONPATH=. pytest tests/test_gui_runner.py tests/test_docs_examples.py -vv`
- 전체 `env PYTHONPATH=. pytest`

#### 완료 기준

- runtime/setup failure와 conversion failure가 GUI summary/log에서 구분된다.
- 오류 메시지는 raw traceback이 아니라 사용자가 조치할 수 있는 설명을 포함한다.
- 기존 CLI 테스트와 schema check가 그대로 통과한다.
...
1. Q54: runtime/install failure를 먼저 분리해 사용자가 GUI를 실행조차 못 하는 문제를 줄인다.
2. Q55: 변환 완료 후 품질 확인 UX를 보강해 partial success와 warning을 놓치지 않게 한다.
3. Q56: batch folder 운영성을 보강해 대형 폴더 변환 중단/재시도 리스크를 줄인다.
4. Q57: 실제 비개발자 실행 문서를 정리해 설치와 초기 진단 비용을 낮춘다.
5. Q58: 앞선 GUI 후속 작업의 계약 회귀를 headless smoke와 문서 테스트로 고정한다.

## 완료 명세 Archive

완료된 Q34-Q53 품질 개선 명세와 구현 결과는 `docs/QUALITY_IMPROVEMENT_IMPLEMENTED_SPECS.md`에 보관한다.
