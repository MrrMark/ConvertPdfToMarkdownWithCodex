...
| 순서 | 작업 | 상태 | 완료 기준 |
| --- | --- | --- | --- |
| 5 | Q156. 비교 벤치마크 공정성 | 구현·로컬 검증 완료, PR merge 대기 | 동일 입력·유효 옵션·계측 구간, 독립 프로세스, 준비 비용 분리 |
| 6 | Q157. 외부 CPU 도구 비교 실험 | 구현·실측·로컬 검증 완료, PR merge 대기 | 고정 버전별 실행 결과 또는 명시적 실행 불가 사유 |
| 7 | Q158. 한글 검색 평가 및 회귀 마감 | 원문 대조·Mac/Windows/Linux 검증 완료, 사용자 정답 승인·PR merge 대기 | 한·영 30개 질문, BM25 지표, 전체 회귀 및 Windows CPU CI |
