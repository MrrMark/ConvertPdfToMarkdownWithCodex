...
|---|---|---:|---:|---|
| 2026-06-21 | Q141-Q142 domain recommendation hardening | 100/100 | 0 | `domain_adapter_recommendation.recommendation_basis`가 top/runner-up score, score margin, matched candidate count, ambiguity 여부를 raw content 없이 기록한다. low/ambiguous recommendation은 `recommended_options.domain_adapter`로 승격하지 않는다. `customer-requirements` 추천 coverage를 추가했고, OCP requirement 문서는 OCP로 우선 분류되는 fixture로 오분류를 방어한다. GUI validation은 customer requirement 추천과 storage/customer mixed-domain warning을 검증한다. 관련 tests 통과 및 active quality backlog 없음 |
| 2026-06-21 | Q140 storage domain recommendation coverage | 100/100 | 0 | raw-content-free domain adapter recommendation이 TCG/SPDM/Caliptra뿐 아니라 NVMe/PCIe/OCP technical/storage spec도 식별한다. GUI validation은 `technical_spec_rag + Domain=none`에서 NVMe storage spec 추천 warning을 표시하고, storage/security spec이 섞인 folder에는 mixed-domain warning을 유지한다. recommendation payload는 raw PDF text/table row/image bytes를 포함하지 않는다. 관련 tests 통과 및 active quality backlog 없음 |
| 2026-06-21 | Q136-Q139 domain recommendation and performance preflight | 100/100 | 0 | technical spec domain adapter missing warning이 enum 기반 추천 목록을 사용해 `caliptra` drift를 방지한다. GUI validation은 `technical_spec_rag + Domain=none`에서 TCG/SPDM/Caliptra 보안 스펙 추천 또는 mixed-domain folder warning을 표시하며, 변환 옵션은 자동 변경하지 않는다. Security adapter calibration은 `Authority Name/Authority UID`, `Req/Rsp/Req Code`, `Mailbox Cmd/Register Field` header alias를 domain unit/normalized field까지 검증한다. `pdf2md_plan_large_spec_conversion`은 raw text 없이 `domain_adapter_recommendation`, `performance_profile`, `recommended_options.page_workers`를 반환한다. 관련 tests 통과 및 active quality backlog 없음 |
| 2026-06-21 | Q135 Caliptra security spec adapter and evidence path | 100/100 | 0 | `domain_adapter=caliptra`를 추가해 Caliptra RoT service, asset, threat, mailbox command, register field, measurement/attestation/key unit taxonomy를 `domain_units_rag.jsonl`에 기록하고 SSD-RAG validator에서 `HIL/Caliptra`로 검증한다. `run_latest_ssd_security_spec_benchmark.py --spec-document-type caliptra`는 Caliptra 2.1 / `https://spec.caliptra.io/` metadata와 summary-only `security_domain_unit_counts`를 남기며 raw spec text/Markdown/table row/image/local path는 포함하지 않는다. 관련 schema/docs/tests 통과 및 active quality backlog 없음 |
| 2026-06-21 | Q134 SSD security spec benchmark evidence path | 100/100 | 0 | `run_latest_ssd_security_spec_benchmark.py`를 추가해 SPDM, SPDM to Storage Binding, TCG Storage, PCIe Base 계열 스펙을 `technical_spec_rag + domain_adapter=spdm|tcg|pcie`로 변환하고 SSD-RAG contract summary를 남기는 evidence path를 만들었다. DMTF SPDM 1.4.0/SPDM to Storage Binding 1.0.0 metadata를 기본값으로 제공하며, report는 source hash, option matrix, sidecar count, `security_domain_unit_counts`만 기록하고 raw spec text/Markdown/table row/image/local path를 포함하지 않는다. 관련 schema/docs/tests 통과 및 active quality backlog 없음 |
| 2026-06-20 | Q131-Q133 large spec MCP stability guardrails | 100/100 | 0 | `pdf2md_plan_large_spec_conversion`이 대형 technical spec 변환 전 page-window/profile/image mode/timeout 권고를 raw content 없이 제공한다. `pdf2md_validate_visual_sidecars`와 `visual_sidecar_contract_report.json`은 visual sidecar source linkage와 generated content sidecar-only 계약을 검증한다. page-window merge는 sidecar record/byte inventory와 `merge_memory_guard`를 기록해 과도한 merge 입력을 advisory warning으로 분류한다. 관련 schema/docs/tests 통과 및 active quality backlog 없음 |
| 2026-06-20 | Q127-Q130 agent skill and MCP contract alignment | 100/100 | 0 | `pdf2md-rag-ingest` Skill과 agent guide가 최신 layout/figure OCR/figure semantics/domain metadata sidecar 계약을 안내하도록 정렬했다. MCP `pdf2md_list_profiles`는 OCR backend option을 노출하고, `pdf2md_validate_ssd_rag_contract`는 SSD-RAG validator를 compact/count-only 응답으로 실행한다. Cursor/Continue rule은 canonical Skill references를 가리키는 얇은 adapter로 유지하고 dry-run source mapping으로 regeneration drift를 줄였다. 관련 docs/tests 통과 및 active quality backlog 없음 |
| 2026-06-20 | Q118-Q125 pdf2md native migration closure | 100/100 | 0 | Docling을 runtime dependency로 채택하지 않고 내부 Document IR, table confidence v2, native hybrid chunking v2, layout sidecar, region OCR evidence, OCR backend registry, figure semantics, domain adapter registry를 `pdf2md` 네이티브 계약으로 구현했다. `domain_units_rag.jsonl`은 `adapter_metadata`와 `cross_spec_compatibility`를 포함하고, SSD-RAG validator는 NVMe/OCP/PCIe/TCG/SPDM/customer/manual adapter mapping과 stable source metadata를 검증한다. 최신 NVMe Base, NVM Command Set, OCP Datacenter NVMe SSD smoke/eval 경로와 전체 pytest/CI가 통과했으며 active quality backlog 없음 |
| 2026-06-14 | Q115-Q116 visual technical spec RAG and sidecar handoff closure | 100/100 | 0 | `technical_spec_rag_visual`을 CLI/MCP/GUI/agent-pack workflow에 노출하고, `figure_text`/`figure_description`/`figure_structure` visual chunk 계약과 `figure_descriptions_rag.jsonl`, `figure_structures_rag.jsonl` sidecar를 schema/docs/validator/test로 고정했다. latest NVMe Base/Command/OCP benchmark는 `--visual-mode`와 aggregate-only `visual_eval` metric을 기록한다. `ssd-verification-agent`용 direct sidecar ingest/API/MCP/SpecAnalysisAgent handoff 명세를 분리했고, 실제 agent repo 구현은 별도 후속 작업으로 남긴다. active quality backlog 없음 |
| 2026-06-05 | Q114 Docling layout adapter comparison mode | 100/100 | 0 | Docling-installed benchmark에서 `--layout-comparison-mode summary`를 지원해 current-tool과 Docling의 table/figure/page/text-container layout count를 sanitized metric으로 비교한다. `metric_deltas`에 layout count 차이를 남기되, Docling layout을 `document.md`, table, figure, RAG sidecar에 병합하지 않는다. raw Docling dict/Markdown/PDF text/image/customer path는 저장하지 않으며 active quality backlog 없음 |
| 2026-06-05 | Q113 local-only figure description evaluation pack | 100/100 | 0 | `scripts/evaluate_figure_descriptions.py`와 `figure_description_eval_report.json` schema를 추가해 `figure_descriptions_rag.jsonl`이 local-only, source evidence/source refs, backend non-invocation, retrieval chunk linkage 계약을 지키는지 검사한다. release gate `--gates figure-description-eval`을 추가했고, report는 raw image/PDF text/customer path를 포함하지 않는다. VLM/Docling picture description 생성은 여전히 제외한다. active quality backlog 없음 |
| 2026-06-05 | Q112 region OCR report-only prototype | 100/100 | 0 | `--figure-region-ocr`가 켜진 경우 figure bbox를 로컬 렌더/crop OCR로 시도하고, 결과를 `figures_rag.jsonl` diagnostics와 report summary counter에만 기록하도록 확장했다. `region_ocr.report_only=true`, `text_replaced=false`로 원문 Markdown/text extraction 비대체 원칙을 고정했다. runtime unavailable, invalid bbox, OCR failure는 conversion 실패가 아니라 rejected/runtime diagnostic으로 남긴다. active quality backlog 없음 |
| 2026-06-05 | Q111 OCR backend adapter contract | 100/100 | 0 | 기본 OCR 출력 품질을 바꾸지 않고 `pdf2md/extractors/ocr_backends/` adapter contract를 추가했다. 변환 경로는 여전히 `tesseract`만 허용하며, `pytesseract + pypdfium2` 호출을 tesseract adapter로 감쌌다. `Config.ocr_backend`, CLI `--ocr-backend`, manifest options, OCR warning details에 backend provenance를 기록한다. RapidOCR/EasyOCR/OCRmac/Docling 실행 구현과 GUI 선택 UI는 다음 evidence 확보 전까지 제외했다. active quality backlog 없음 |
| 2026-06-05 | Q110 multi OCR backend runtime probe | 100/100 | 0 | `scripts/probe_ocr_backends.py`와 `ocr_backend_probe_report.json` schema를 추가해 Tesseract, RapidOCR, EasyOCR, OCRmac, Docling backend의 module/executable/language/platform readiness를 raw content 없이 점검할 수 있게 했다. release gate `--gates ocr-backends`는 기본적으로 ready backend가 없으면 실패하며, 변환 기본 OCR 경로는 변경하지 않았다. active quality backlog 없음 |
| 2026-06-05 | Q109 Docling-installed benchmark gate | 100/100 | 0 | Q108 latest NVMe evidence path를 Docling 설치 환경에서 강제 검증할 수 있도록 `--require-docling`, latest wrapper `--fail-on-error`, release gate `--gates docling`을 추가했다. 기본 Docling 미설치 advisory skip은 유지하되, gate 모드에서는 `docling_required_not_available` error로 실패한다. 로컬 smoke에서는 최신 NVMe PDF 1페이지로 `passed_release_gate=false`, `exit_code=1`을 확인했다. Docling은 여전히 optional dependency이며 raw Markdown/dict/image/customer path는 저장하지 않는다. active quality backlog 없음 |
| 2026-06-05 | Q108 latest NVMe command set benchmark evidence path | 100/100 | 0 | 공식 최신 NVMe NVM Command Set Rev. 1.2 PDF를 repo 밖 `/tmp`에서 full conversion으로 검증했다. current-tool은 158페이지 `status=success`, `duration_ms=63357`, `pages/sec=2.4956`, `actionable_warning_count=0`, `table_low_quality_count=0`, artifact/index/provenance validator pass를 기록했다. `scripts/run_latest_nvme_command_set_eval.py`는 Q107 figure semantics 옵션을 current-tool benchmark에 전달하고, `latest_nvme_command_set_scorecard.md`에 sanitized metric을 남긴다. Docling 미설치 환경은 `docling_not_installed` advisory skip으로 보존한다. active quality backlog 없음 |
