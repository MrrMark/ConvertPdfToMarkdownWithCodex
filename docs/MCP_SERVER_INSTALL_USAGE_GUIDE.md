...
3. 대형 technical spec이면 `pdf2md_plan_large_spec_conversion`으로 window/profile/image mode, domain adapter recommendation, performance profile 권고를 확인한다.
...
1. `pdf2md_plan_large_spec_conversion`으로 보수적 권고값과 `recommended_options.page_workers`를 확인한다.
