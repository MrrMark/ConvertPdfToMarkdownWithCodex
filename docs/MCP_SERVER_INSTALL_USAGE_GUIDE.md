...
5. SSD technical spec ingest가 목적이면 `pdf2md_validate_ssd_rag_contract`로 domain adapter와 stable metadata 계약을 추가 검증한다.
5. `pdf2md_inspect_report`로 warning을 triage한다.
6. 필요할 때만 artifact URI의 파일을 열어 원문을 확인한다.

대형 PDF 권장 순서:

1. `pdf2md_plan_page_windows`로 window plan을 확인한다.
2. `pdf2md_convert_pdf_windowed`로 변환/검증/merge를 한 번에 실행하거나,
   장애 격리가 필요하면 `pdf2md_convert_page_window`를 window별로 호출한다.
3. `pdf2md_merge_window_outputs`로 최종 산출물을 merge한다.
4. `page_window_merge_report.json`, `conversion_state.json`, `report.json`을 확인한다.
