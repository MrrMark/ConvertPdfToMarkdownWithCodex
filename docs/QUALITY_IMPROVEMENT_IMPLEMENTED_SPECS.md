...
- Q97: modern Python tooling and packaging readiness
- Q98: lazy structure marker OCR
- Q99: page worker chunked parallelization
- Q100: OCR page parallelization
- Q101: table strategy adaptive mode
- Q102: fast output profile and sidecar scope
- Q103: assetless technical RAG figure text chunks
- Q104: Docling benchmark harness and comparison pack
- Q105: Docling-informed OCR and layout extension design
- Q106: GUI assetless RAG preset and manual domain adapter
- Q107: assetless figure visual semantics layer
- Q108: latest NVMe command set benchmark evidence path
- Q109: Docling-installed benchmark gate
- Q110: multi OCR backend runtime probe
- Q111: OCR backend adapter contract
- Q112: region OCR report-only prototype
- Q113: local-only figure description evaluation pack
- Q114: Docling layout adapter comparison mode
...
- Q118-Q125: native pdf2md migration closure
- Q126: GUI batch completion summary bounds
- Q127-Q130: Agent Skill, client rule, MCP discovery, and SSD-RAG validator contract alignment
- Q131-Q133: large spec preflight, visual sidecar validator, and page-window merge memory guard
- Q134: latest SSD security spec benchmark evidence path
- Q135: Caliptra security spec adapter and evidence path
- Q136-Q139: domain recommendation, GUI security-domain warning, security adapter calibration, and performance profile preflight
- Q140: storage domain recommendation coverage
- Q141-Q142: domain recommendation basis, ambiguity guard, and customer requirement coverage
- Q143: real spec regression evidence pack expansion
...
- Q148: conversion journal artifact inventory throttling
- Q149: streaming RAG sidecar writers
- Q150: PRD and public docs CLI contract alignment
- Q151: incremental Ruff rule expansion
...
## P2 / Q151. Incremental Ruff Rule Expansion

### 목표

syntax/undefined-name 중심의 최소 Ruff smoke gate를 유지하되, low-risk rule family를 한 단계 확장한다.
광범위한 import sorting/pyupgrade churn은 후속 작업으로 남기고, public artifact, schema, golden output 변경 없이
현재 코드베이스에서 즉시 통과 가능한 단순화 규칙을 품질 게이트에 포함한다.

### 구현 결과

- `pyproject.toml` Ruff lint `select`에 `SIM` rule family를 추가했다.
- baseline 조사 결과 import sorting(`I`), pyupgrade(`UP`), bugbear(`B`)는 현재 변경량 또는 판단 비용이 커서 이번 범위에서 제외했다.
- `SIM` 위반 6곳을 behavior-preserving 형태로 정리했다.
  - table adaptive skip predicate
  - Tk window cleanup suppress
  - GUI recent-state clear suppress
  - table OCR eligibility predicate
  - RAG text block code-line predicate
  - pipeline timeout test monotonic fallback
- 별도 per-file-ignore 없이 전체 repo Ruff 검사를 통과하도록 유지했다.

### 검증

- `.venv311/bin/python -m ruff check . --select SIM`
- `.venv311/bin/python -m ruff check .`
- `.venv311/bin/python -m pytest tests/test_tables.py tests/test_gui_runner.py tests/test_gui_state.py tests/test_rag_figures.py tests/test_rag_text_blocks.py tests/test_pipeline_reporting.py -q`

### 비범위

- import sorting 전체 적용
- pyupgrade 전체 적용
- bugbear rule family 도입
- public behavior, schema, golden output 변경

## P1 / Q150. PRD and Public Docs CLI Contract Alignment

### 목표

PRD와 공개 문서의 CLI 옵션 설명을 현재 구현된 figure semantics/sidecar 계약과 맞춘다. 초기 설계의
`--describe-images` 계열 primary 안내를 제거하고, 현재 CLI의 opt-in sidecar 옵션을 source of truth에 반영한다.

### 구현 결과

- `PRD_pdf_to_markdown_converter.md`의 이미지 설명 생성 정책을 현재 CLI 기준으로 갱신했다.
  - `--rag-generated-figure-descriptions`
  - `--figure-region-ocr`
  - `--figure-description-backend local-vlm|docling`
  - `--figure-structure-extraction`
  - `figure_descriptions_rag.jsonl`, `figure_structures_rag.jsonl`, retrieval chunk sidecar 계약
- PRD 주요 옵션 목록에 현재 CLI의 image/table/RAG/OCR/page worker/timeout/figure semantics 옵션을 반영했다.
- README Development 섹션에 가상환경 활성화 전제와 `.venv311/bin/python -m ...` 또는 `python3 -m ...` 대체 실행 방식을 명시했다.
- `tests/test_docs_examples.py`에 PRD가 현재 figure semantics option label을 포함하고 legacy option을 남기지 않는 계약을 추가했다.

### 검증

- `.venv311/bin/python -m pytest tests/test_docs_examples.py -q`
- `.venv311/bin/python -m pdf2md --help`
- `.venv311/bin/python -m pdf2md.gui --help`
- `.venv311/bin/python -m ruff check tests/test_docs_examples.py`
- `git diff --check`
...
- 새 이미지 설명 backend 구현
- 외부 VLM/API 호출
- Markdown 본문에 generated description을 삽입하는 옵션 추가

## P1 / Q149. Streaming RAG Sidecar Writers

### 목표

대형 technical spec RAG sidecar를 파일로 쓸 때 전체 JSONL 문자열을 한 번에 materialize하지 않고 record 단위로 기록한다.
기존 JSONL payload, 순서, UTF-8/LF, record count, file count 계약은 유지한다.

### 구현 결과

- `pdf2md.utils.io.write_jsonl()` helper를 추가해 JSONL records를 streaming 방식으로 기록한다.
- `pdf2md/output_writers.py`의 RAG sidecar writer를 `write_jsonl()`로 전환했다.
  - `text_blocks_rag.jsonl`
  - `semantic_units_rag.jsonl`, `requirements_rag.jsonl`, `cross_refs_rag.jsonl`
  - `retrieval_chunks_rag.jsonl`
  - `page_layout_rag.jsonl`
  - `figures_rag.jsonl`, `figure_ocr_evidence_rag.jsonl`, `figure_descriptions_rag.jsonl`, `figure_structures_rag.jsonl`
  - `domain_units_rag.jsonl`
  - `requirement_traceability_rag.jsonl`
  - `technical_tables_rag.jsonl`
  - `tables_rag.jsonl`
- 기존 serializer 함수는 외부 호출/테스트 호환을 위해 유지했다.
- `tests/test_output_writers.py`를 추가해 streaming writer 출력이 기존 serializer 문자열과 byte-for-byte 동일함을 검증한다.

### 검증

- `.venv311/bin/python -m pytest tests/test_output_writers.py -q`
- `.venv311/bin/python -m pytest tests/test_golden_corpus.py tests/test_output_schema_contract.py tests/test_artifact_integrity_validator.py tests/test_pipeline_smoke.py -q`
- `.venv311/bin/python scripts/benchmark_conversion.py --output-dir /private/tmp/pdf2md-q149-benchmark --page-counts 10`
- `.venv311/bin/python -m ruff check pdf2md/output_writers.py pdf2md/utils/io.py tests/test_output_writers.py`
- `git diff --check`

### 비범위

- sidecar record schema 변경
- JSONL 압축 또는 sidecar split
- RAG chunk 생성 알고리즘 변경

## P1 / Q148. Conversion Journal Artifact Inventory Throttling

### 목표

대형 스펙 변환 중 page/image progress 또는 stage update마다 `conversion_state.json` 작성을 위해 출력 디렉터리 전체를
재귀 스캔하지 않도록 한다. running state update는 가볍게 유지하고, 완료/중단 상태에서만 전체 artifact inventory를 기록한다.

### 구현 결과

- `ConversionJournal.write_state()`에 `include_artifacts` 플래그를 추가했다.
  - 기본 running state write는 `artifacts_written=[]`로 기록하고 `output_dir.rglob("*")`를 호출하지 않는다.
  - 성공 완료 `mark_finished()`와 fatal/interrupted final state write는 `include_artifacts=True`로 전체 inventory를 유지한다.
- artifact inventory scan을 `_scan_artifacts_written()`으로 분리해 호출 회귀를 테스트할 수 있게 했다.
- `interrupted_report.json`과 report summary의 `artifacts_written` 계약은 기존처럼 full scan 기반 deterministic sorted inventory를 유지한다.
- `tests/test_pipeline_reporting.py`에 progress/stage update 중 artifact scan이 발생하지 않고 terminal state에서만 scan이 발생하는 회귀 테스트를 추가했다.

### 검증

- `.venv311/bin/python -m pytest tests/test_pipeline_reporting.py::test_conversion_journal_defers_artifact_scan_until_terminal_state tests/test_pipeline_smoke.py::test_pipeline_generates_outputs tests/test_pipeline_smoke.py::test_pipeline_writes_interrupted_report_after_partial_artifact -q`
- `.venv311/bin/python -m ruff check pdf2md/pipeline.py tests/test_pipeline_reporting.py`
- `git diff --check`

### 비범위

- conversion journal schema field 변경
- progress event public payload 변경
- output artifact 파일명/위치 변경

## P1 / Q147. Security Visual Sidecar Fixture Coverage

### 목표

`technical_spec_rag_visual`의 visual sidecar 품질을 SPDM sequence diagram, TCG architecture diagram, Caliptra RoT/block diagram 같은 security spec 그림 관점에서 검증한다. 생성/파생 visual text는 sidecar-only 계약을 유지하고, Markdown 본문에는 삽입하지 않는다.

### 구현 결과

- `tests/fixtures/pdf_builder.py`에 deterministic security diagram PDF fixture helper를 추가했다.
- `tests/test_rag_figures.py`가 `technical_spec_rag_visual + image_mode=placeholder` 조합으로 SPDM/TCG/Caliptra diagram 변환 smoke를 검증한다.
  - `figures_rag.jsonl`, `figure_ocr_evidence_rag.jsonl`, `figure_descriptions_rag.jsonl`, `figure_structures_rag.jsonl`, `retrieval_chunks_rag.jsonl`가 모두 생성되고 source figure provenance를 유지한다.
  - fake region OCR text는 `figure_ocr_evidence_rag.jsonl`와 visual sidecar/chunk에만 남고 Markdown 본문에는 삽입되지 않는다.
- `tests/test_visual_sidecar_contract.py`가 security diagram bundle을 raw-content-free validator report로 통과시키는 계약을 추가했다.
- `tests/test_rag_chunks.py`가 security visual description/structure chunk의 source refs를 figure 및 sidecar provenance까지 검증한다.
- `docs/OUTPUT_SCHEMA.md`와 `docs/RAG_INDEXER_INTEGRATION_RECIPES.md`에 security visual fixture coverage를 문서화했다.

### 검증

- `python3 -m pytest tests/test_rag_figures.py::test_security_visual_diagram_fixtures_emit_sidecars_without_markdown_pollution tests/test_visual_sidecar_contract.py::test_validate_visual_sidecar_contract_accepts_security_diagram_bundle_without_raw_text tests/test_rag_chunks.py::test_retrieval_chunks_link_security_visual_semantics_to_sidecar_sources -q`
- `python3 -m pytest tests/test_rag_figures.py tests/test_visual_sidecar_contract.py tests/test_rag_chunks.py -q`
- `git diff --check`

### 비범위

- 외부 VLM/API 호출
- 생성 설명을 authoritative source text로 취급
- 이미지 자체를 대량 fixture로 커밋

## P1 / Q146. Large Spec Plan Apply Workflow

### 목표

대형 NVMe/OCP/TCG/SPDM/Caliptra 스펙 변환 전에 생성한 `large_spec_preflight_plan` 권고를 사용자가 안전하게 적용할 수 있는 opt-in workflow를 제공한다. 수동 옵션 복사 과정에서 domain adapter, image mode, page workers, timeout, window size가 어긋나는 실수를 줄이되, 직접 지정한 변환 옵션은 항상 우선한다.

### 구현 결과

- `pdf2md/preflight.py`에 plan 적용 helper와 `plan_apply_report.json` 계약을 추가했다.
  - 적용 대상은 `rag_profile`, `domain_adapter`, `image_mode`, `rag_sidecar_scope`, `page_workers`, image/figure timeout, MCP windowed `window_size`로 제한한다.
  - 직접 옵션은 `explicit_option_precedence`로 스킵하고, 적용 전/후 `option_matrix`를 기록한다.
  - low/ambiguous `domain_adapter_recommendation`은 자동 적용하지 않는다.
- CLI에 `--apply-plan`을 추가해 단일 PDF 변환에서 preflight plan을 opt-in으로 적용한다.
- MCP `pdf2md_convert_pdf`와 `pdf2md_convert_pdf_windowed`에 `apply_plan_path`를 추가했다.
  - 단일 변환은 적용 audit와 `plan_apply_report_uri`를 반환한다.
  - windowed 변환은 `window_size` 권고까지 적용한 뒤 page-window plan/convert/merge를 실행한다.
- `docs/OUTPUT_SCHEMA.md`와 `docs/schema/plan_apply_report.schema.json`에 raw-content-free audit 계약을 문서화했다.

### 검증

- `python3 -m pytest tests/test_preflight.py::test_large_spec_plan_apply_maps_safe_recommendations_without_raw_content tests/test_preflight.py::test_large_spec_plan_apply_rejects_low_ambiguous_domain_adapter tests/test_cli.py::test_cli_apply_plan_maps_preflight_options_and_writes_audit tests/test_cli.py::test_cli_apply_plan_keeps_explicit_option_precedence tests/test_mcp_server.py::test_convert_pdf_apply_plan_maps_options_and_returns_audit tests/test_mcp_server.py::test_convert_pdf_windowed_apply_plan_uses_recommended_window_size -q`
- `python3 -m pytest tests/test_preflight.py tests/test_cli.py tests/test_mcp_server.py tests/test_docs_examples.py tests/test_output_schema_contract.py -q`
- `git diff --check`

### 비범위

- 사용자의 명시적 opt-in 없는 자동 옵션 변경
- raw sample text, table row, image bytes 저장
- 외부 job scheduler/cloud execution
- Q147의 security visual sidecar fixture coverage

## P1 / Q145. Security Spec Text-Derived Domain Candidate Layer

### 목표

TCG/SPDM/Caliptra 보안 스펙에서 표로 추출되지 않는 본문/목록/heading 기반 domain signal을 보수적으로 기록한다. 기존 table-derived `domain_units_rag.jsonl` 계약은 확정 산출물로 유지하고, 본문 기반 후보는 `candidate_status=review_only` 계층으로 분리한다.

### 구현 결과

- `pdf2md/serializers/rag_domain_adapters.py`가 TCG/SPDM/Caliptra text block에서 source-derived review-only candidate를 생성한다.
  - 후보는 `candidate_status="review_only"`, `candidate_kind`, `review_required=true`, `review_reasons[]`, `candidate_evidence.source_text_span`을 포함한다.
  - `text`는 원문 text block 그대로 유지하고, `description`은 생성/요약하지 않는다.
  - source refs는 `text_block` provenance만 사용해 table-derived unit과 구분한다.
- `pdf2md/serializers/rag_chunks.py`가 review-only domain candidate를 낮은 retrieval priority와 `review_only` semantic type으로 chunk화한다.
- `scripts/validate_ssd_rag_contract.py`가 review-only 후보를 확정 domain coverage와 required normalized field 검증에서 제외한다.
- `docs/OUTPUT_SCHEMA.md`에 review-only domain candidate 계약을 문서화했다.

### 검증

- `python3 -m pytest tests/test_rag_domain_adapters.py::test_security_text_domain_candidates_are_review_only_and_source_derived tests/test_rag_chunks.py::test_retrieval_chunks_keep_review_only_domain_candidates_low_priority tests/test_ssd_rag_contract.py::test_ssd_rag_contract_does_not_treat_review_only_domain_candidates_as_confirmed_units tests/test_pipeline_smoke.py::test_pipeline_emits_review_only_security_text_domain_candidates -q`
- `python3 -m pytest tests/test_rag_domain_adapters.py tests/test_rag_chunks.py tests/test_ssd_rag_contract.py tests/test_pipeline_smoke.py tests/test_docs_examples.py -q`
- `git diff --check`
...
- Q146의 large spec plan apply workflow

## P1 / Q144. Wide Technical Table Header Semantics

### 목표

NVMe reservation/status/register, OCP compliance matrix처럼 넓고 다층 header를 가진 기술 표에서 `Column N` fallback으로 사라지던 parent/child header context를 RAG sidecar에 보존한다. Markdown 출력은 기존 보수 정책을 유지하고, 복잡 표는 계속 HTML fallback을 우선한다.

### 구현 결과

- `pdf2md/extractors/tables.py`가 table diagnostics와 `tables_rag.jsonl` row에 `column_header_paths`, `column_placeholder_header_ratio`, `stub_column_headers`를 기록한다.
  - multi-row header는 parent/child `path`와 `path_text`를 보존한다.
  - `Column N` placeholder에는 가능한 `inferred_parent_header`와 `neighbor_headers`를 함께 기록한다.
  - table quality/debug payload에 placeholder header ratio를 남겨 triage 신호로 활용할 수 있다.
- `technical_tables_rag.jsonl`, `domain_units_rag.jsonl`, `retrieval_chunks_rag.jsonl.context_metadata`까지 header lineage를 source-derived metadata로 전파한다.
- contextual embedding prefix에는 `Header paths:`를 추가해 table-like chunk 검색 시 flattened header보다 풍부한 문맥을 사용할 수 있게 했다.
- `docs/OUTPUT_SCHEMA.md`에 tables/technical/retrieval chunk header lineage 필드 계약을 문서화했다.

### 검증

- `python3 -m pytest tests/test_tables.py::test_multi_row_header_lineage_preserves_parent_child_paths tests/test_tables.py::test_placeholder_header_lineage_keeps_parent_and_neighbor_context -q`
- `python3 -m pytest tests/test_rag_technical_tables.py::test_technical_tables_extract_official_command_set_header_variants tests/test_rag_domain_adapters.py::test_nvme_domain_adapter_extracts_command_and_register_units tests/test_rag_chunks.py::test_retrieval_chunks_include_text_semantic_requirement_and_table_provenance -q`
- `python3 -m pytest tests/test_tables.py tests/test_rag_tables.py tests/test_rag_technical_tables.py tests/test_rag_domain_adapters.py tests/test_rag_chunks.py tests/test_docs_examples.py -q`
- `git diff --check`

### 비범위

- 복잡 표를 억지로 GFM으로 변환
- 신규 table extraction engine 도입
- 셀 내용 추정/보정 또는 원문 의미 변경
- Q145의 본문/목록 기반 security domain candidate layer

## P1 / Q143. Real Spec Regression Evidence Pack Expansion

### 목표

NVMe/OCP/TCG/SPDM/Caliptra 변환 품질을 실제 또는 redacted spec slice 실행 결과에 가까운 count/coverage evidence로 반복 검증한다. public report에는 raw PDF text, Markdown body, table row content, image bytes, local path를 포함하지 않는다.

### 구현 결과

- `scripts/run_ssd_corpus_profile.py`에 domain coverage summary를 추가했다.
  - `domain_units_rag.jsonl`과 `technical_tables_rag.jsonl`에서 unit type count를 집계한다.
  - adapter registry와 profile expectation에 정의된 normalized field 이름만 coverage 대상으로 삼고, field 값은 report에 저장하지 않는다.
  - `coverage_expectations` 또는 `preset=default`로 family별 minimum unit type count와 normalized field coverage threshold를 opt-in 검증한다.
- `local_corpus_evidence_pack.json`에 문서/도메인별 `coverage_summary`, `coverage_failure_count`, `raw_content_included=false`, `table_row_content_included=false`, `image_bytes_included=false`, `local_paths_included=false` 계약을 추가했다.
- coverage 위반은 `coverage_failure` signature로 그룹화되어 어느 `domain_units_rag.jsonl` metric이 부족한지 식별된다.
- `LocalCorpusEvidencePack` 모델과 `docs/schema/local_corpus_evidence_pack.schema.json` public schema를 갱신했다.

### 검증

- `python3 -m pytest tests/test_ssd_rag_contract.py tests/test_corpus_evidence_tools.py tests/test_output_schema_contract.py -q`
...
- 생성형 모델 기반 평가
- Q144의 wide/multi-row table header lineage 개선

## P1 / Q141-Q142. Domain Recommendation Hardening

### 목표

Q136-Q140 domain adapter recommendation을 운영에서 더 안전하게 쓰도록 raw-content-free 판단 근거와 ambiguity guard를 추가하고, customer requirement 문서 추천 coverage를 보강한다.

### 구현 결과

- `domain_adapter_recommendation.recommendation_basis`를 추가해 top adapter/score, runner-up adapter/score, score margin, matched candidate count, ambiguity 여부를 raw content 없이 기록한다.
- low/ambiguous recommendation은 `recommended_options.domain_adapter`에 자동 권고하지 않는 계약을 fixture로 고정했다.
- `customer-requirements` keyword/anchor scoring을 추가해 customer/vendor requirement, acceptance criteria, compliance matrix 성격 문서를 추천한다.
- OCP requirement 문서가 `customer-requirements`로 오분류되지 않고 `ocp`로 남도록 ranking fixture를 추가했다.
- GUI validation에서 customer requirement 추천 warning과 storage/customer mixed-domain folder warning을 검증했다.

### 검증

- `python3 -m pytest tests/test_preflight.py::test_large_spec_preflight_records_recommendation_basis_without_raw_text tests/test_preflight.py::test_large_spec_preflight_does_not_promote_ambiguous_domain_adapter -q`
- `python3 -m pytest tests/test_preflight.py::test_large_spec_preflight_recommends_customer_requirements_without_raw_text tests/test_preflight.py::test_large_spec_preflight_keeps_ocp_ahead_of_customer_requirements -q`
- `python3 -m pytest tests/test_gui_runner.py::test_gui_request_recommends_customer_requirements_domain_for_technical_profile tests/test_gui_runner.py::test_gui_request_warns_for_mixed_storage_and_customer_domain_folder -q`

## P1 / Q140. Storage Domain Recommendation Coverage

### 목표

Q136-Q139의 raw-content-free domain adapter recommendation을 보안 스펙 중심에서 NVMe, PCIe, OCP technical/storage spec까지 확장한다.

### 구현 결과

- `pdf2md.preflight` domain recommendation keyword/anchor registry에 `nvme`, `pcie`, `ocp`를 추가했다.
- 추천 scoring은 기존과 동일하게 파일명과 샘플 페이지 keyword count만 사용하고, recommendation payload에는 raw PDF text, Markdown body, table row content, image bytes를 포함하지 않는다.
- `technical_spec_rag + Domain=none` GUI validation warning 경로가 NVMe 같은 storage spec에도 동일하게 동작하도록 fixture를 추가했다.
- 폴더 샘플에 storage/security spec이 섞인 경우 mixed-domain warning을 표시하는 회귀 테스트를 추가했다.

### 검증

- `python3 -m pytest tests/test_preflight.py::test_large_spec_preflight_recommends_storage_domain_adapters_without_raw_text tests/test_preflight.py::test_large_spec_preflight_recommends_security_domain_adapter_without_raw_text -q`
- `python3 -m pytest tests/test_gui_runner.py::test_gui_request_recommends_storage_domain_for_technical_profile tests/test_gui_runner.py::test_gui_request_recommends_security_domain_for_technical_profile tests/test_gui_runner.py::test_gui_request_warns_for_mixed_storage_and_security_domain_folder tests/test_gui_runner.py::test_gui_request_warns_for_mixed_security_domain_folder -q`

## P1 / Q136-Q139. Domain Recommendation And Performance Profile Follow-up

### 목표

TCG/SPDM/Caliptra 보안 스펙 도메인 선택 UX와 대형 technical spec preflight 성능 권고를 보강한다.

### 구현 결과

- technical spec RAG profile에서 domain adapter가 `none`일 때 warning details의 `recommended_domain_adapters`를 `DomainAdapterMode` enum 기반으로 생성해 `caliptra` 누락 같은 drift를 막았다.
- `recommend_domain_adapter_for_pdf()`와 `pdf2md_plan_large_spec_conversion`의 `domain_adapter_recommendation`을 추가했다. 추천은 파일명과 샘플 페이지 키워드 count만 사용하고 raw PDF text, Markdown body, table cell content, image bytes를 출력하지 않는다.
- GUI validation에서 `technical_spec_rag` 계열과 `Domain=none` 조합일 때 보안 스펙 추천을 warning으로 표시한다. 폴더 샘플에서 TCG/SPDM/Caliptra가 섞이면 한 domain adapter를 전체 폴더에 적용하지 않도록 mixed-domain warning을 표시한다.
- TCG/SPDM/Caliptra domain adapter가 `Authority Name/Authority UID`, `Req/Rsp/Req Code`, `Mailbox Cmd/Register Field` 같은 일반 header alias를 인식하도록 calibration fixture를 보강했다.
- 대형 spec preflight에 `performance_profile`과 `recommended_options.page_workers`를 추가해 sample page의 page count/table/image density 기반으로 guarded parallelism을 권고한다.

### 검증

- `python3 -m pytest tests/test_cli.py::test_cli_technical_spec_profile_without_domain_adapter_emits_advisory -q`
- `python3 -m pytest tests/test_preflight.py::test_large_spec_preflight_recommends_security_domain_adapter_without_raw_text tests/test_preflight.py::test_large_spec_preflight_recommends_performance_profile_from_sample_metrics -q`
- `python3 -m pytest tests/test_gui_runner.py::test_gui_request_recommends_security_domain_for_technical_profile tests/test_gui_runner.py::test_gui_request_warns_for_mixed_security_domain_folder -q`
- `python3 -m pytest tests/test_rag_domain_adapters.py::test_security_domain_adapters_accept_common_header_aliases -q`

## P1 / Q135. Caliptra Security Spec Adapter And Evidence Path

### 목표

TCG/SPDM과 같이 SSD 보안/Root of Trust 분석에 연결되는 Caliptra spec을 first-class domain adapter와 latest security benchmark path에 포함한다.

### 구현 결과

- `DomainAdapterMode.CALIPTRA = "caliptra"`를 추가하고 CLI/GUI/MCP/profile discovery/schema 계약에 반영했다.
- `pdf2md.serializers.rag_domain_adapters`에 Caliptra header tokens, unit taxonomy, normalized field registry를 추가했다.
- Caliptra unit type으로 `caliptra_rot_service`, `caliptra_asset`, `caliptra_threat`, `caliptra_interface`, `caliptra_mailbox_command`, `caliptra_register_field`, `caliptra_security_state`, `caliptra_measurement`, `caliptra_attestation`, `caliptra_crypto_key`를 추가했다.
- SSD contract validator에 `caliptra -> HIL/Caliptra` mapping과 Caliptra security unit validation을 추가했다.
- `scripts/run_latest_ssd_security_spec_benchmark.py`에 `--spec-document-type caliptra`를 추가하고, 기본 metadata를 `Caliptra 2.1`, `https://spec.caliptra.io/`로 기록한다.
- Caliptra synthetic fixture와 benchmark/adapter/contract tests를 추가했고, report는 raw spec text, Markdown body, retrieved text, table row content, image bytes, local input path를 포함하지 않는 정책을 유지한다.

### 검증

- `python3 -m pytest tests/test_rag_domain_adapters.py::test_domain_adapter_registry_exposes_ssd_agent_contract_metadata tests/test_rag_domain_adapters.py::test_caliptra_domain_adapter_extracts_rot_assets_and_mailbox_units tests/test_ssd_rag_contract.py::test_ssd_rag_contract_accepts_caliptra_first_class_spec_type tests/test_quality_gate_scripts.py::test_latest_ssd_security_spec_benchmark_writes_sanitized_caliptra_summary tests/test_quality_gate_scripts.py::test_latest_ssd_security_spec_benchmark_distinguishes_spec_families -q`
- `python3 scripts/export_output_schema.py --check`

## P1 / Q134. Latest SSD Security Spec Benchmark Evidence Path

### 목표

NVMe/OCP 중심 latest evidence path를 SPDM, SPDM to Storage Binding, TCG Storage, PCIe 같은 SSD 인접 security/register 스펙으로 확장한다.

### 구현 결과

- `scripts/run_latest_ssd_security_spec_benchmark.py`를 추가했다.
- wrapper는 `spdm`, `spdm-storage-binding`, `tcg-storage`, `pcie-base` spec document type을 지원하고, 각 type을 `domain_adapter=spdm|tcg|pcie`와 SSD-RAG contract validator에 연결한다.
- 기본 SPDM metadata는 DMTF 공식 SPDM 페이지 기준 SPDM 1.4.0과 SPDM to Storage Binding 1.0.0을 사용한다.
- `latest_ssd_security_spec_benchmark_report.json`과 scorecard는 source URL, source SHA-256, option matrix, sidecar size/count, `security_domain_unit_counts`, sanitized contract summary만 기록한다.
- report/schema/test는 raw spec text, Markdown body, retrieved text, table row content, image bytes, local input path를 포함하지 않는 정책을 고정한다.

### 검증

- `python3 -m pytest tests/test_quality_gate_scripts.py::test_latest_ssd_security_spec_benchmark_writes_sanitized_spdm_summary tests/test_quality_gate_scripts.py::test_latest_ssd_security_spec_benchmark_distinguishes_spec_families -q`
- `python3 scripts/export_output_schema.py --check`

## P1 / Q133. Page-Window Sidecar Merge Memory Guard

### 목표

대형 spec window merge 전에 sidecar record/byte 규모를 집계해, merge가 과도하게 커질 때 실패가 아니라 advisory warning과 운영 metadata로 남긴다.

### 구현 결과

- `merge_window_outputs()`가 window별 public sidecar inventory를 계산해 `page_window_merge_report.json.sidecar_inventory`와 compact MCP 응답에 기록한다.
- `merge_record_warning_threshold`, `merge_bytes_warning_threshold`를 `pdf2md_merge_window_outputs`, `pdf2md_convert_pdf_windowed`까지 노출했다.
- threshold 초과 시 `page_window_merge_record_guard`, `page_window_merge_byte_guard` warning을 남기고 `merge_memory_guard`에 초과 여부를 기록한다.
- raw Markdown/PDF text나 full sidecar record body 없이 count/byte metadata만 반환하도록 테스트로 고정했다.
...
## P1 / Q132. Visual Sidecar Contract Validator

### 목표

`figure_ocr_evidence_rag.jsonl`, `figure_descriptions_rag.jsonl`, `figure_structures_rag.jsonl`, `page_layout_rag.jsonl`이 source figure와 안전하게 연결되고 generated content가 Markdown 본문에 삽입되지 않는지 local-only로 검증한다.

### 구현 결과

- `scripts/validate_visual_sidecar_contract.py`를 추가하고 `visual_sidecar_contract_report.json`을 생성한다.
- MCP `pdf2md_validate_visual_sidecars`와 내부 `validate_visual_sidecars_output()` wrapper를 추가했다.
- validator는 page layout region count, figure OCR source_refs, generated description sidecar-only contract, structure source_refs를 검증한다.
- MCP 응답은 `summary`, 제한된 `findings_preview`, report/artifact URI만 반환하고 raw caption/OCR/description text를 반환하지 않는다.

### 검증

- `python3 -m pytest tests/test_visual_sidecar_contract.py tests/test_mcp_server.py -q`
- `python3 scripts/export_output_schema.py --check`

## P1 / Q131. Large Spec Preflight Planner

### 목표

NVMe Base 같은 대형 technical spec 변환 전에 full conversion 없이 보수적인 page-window/profile/image mode/timeout 권고를 생성한다.

### 구현 결과

- `pdf2md/preflight.py`에 `PreflightOptions`와 `plan_large_spec_conversion()`을 추가했다.
- `scripts/plan_large_spec_conversion.py` CLI와 MCP `pdf2md_plan_large_spec_conversion`을 추가했다.
- page count와 sample page의 text/image/table density를 기반으로 `technical_spec_rag`, `technical_spec_rag_visual`, `image_mode`, `rag_sidecar_scope`, `window_size`, timeout 권고를 생성한다.
- raw PDF text/table cell/image bytes 없이 source hash, sampled page 번호, aggregate estimates만 반환한다.

### 검증

- `python3 -m pytest tests/test_preflight.py tests/test_mcp_server.py -q`

## P1 / Q130. Agent Rule Regeneration And Drift Guard

### 목표

canonical Skill 수정 후 Cursor/Continue 등 client rule target이 오래된 지침으로 남지 않게 한다.

### 구현 결과

- `scripts/install_agent_skill_pack.py` dry-run/설치 출력이 `target <- source` 형태로 source-to-target mapping을 표시한다.
- `docs/AGENT_SKILL_PORTABILITY.md`와 `docs/AGENT_SKILL_USAGE_GUIDE.md`에 `--overwrite --dry-run` 기반 재생성 절차를 추가했다.
- Cursor/Continue adapter template은 full sidecar 계약을 중복 복사하지 않고 canonical Skill reference를 가리키도록 보강했다.
- `tests/test_agent_skill_pack.py`가 canonical Skill link, artifact reference link, dry-run source mapping을 검증한다.
...
## P1 / Q129. MCP SSD-RAG Contract Validator Tool

### 목표

MCP client가 변환 후 `scripts/validate_ssd_rag_contract.py`를 shell 조립 없이 실행할 수 있게 한다.

### 구현 결과

- `pdf2md_validate_ssd_rag_contract` MCP tool과 내부 `validate_ssd_rag_contract_output()` wrapper를 추가했다.
- validator report는 `ssd_rag_contract_report.json`으로 저장하고 MCP 응답은 `report_uri`, `summary`, `errors_preview`, `warnings_preview`, artifact URI만 반환한다.
- MCP 응답에는 `sample_mapped_chunk`, raw spec text, full Markdown body, table row content, image bytes를 포함하지 않는다.
- MCP 개발/사용 문서와 agent guide에 generic artifact validator와 SSD-RAG contract validator의 역할 분리를 반영했다.

### 검증

- `python3 -m pytest tests/test_mcp_server.py -q`
- `python3 -m pytest tests/test_ssd_rag_contract.py -q`

## P0 / Q128. MCP Profile Discovery Contract Alignment

### 목표

MCP client가 `pdf2md_list_profiles`만으로 profile/adapter/output mode와 OCR backend option을 발견하게 한다.

### 구현 결과

- `pdf2md_list_profiles()` 응답에 `ocr_backends`를 추가했다.
- `pdf2md_doctor`는 runtime readiness, `pdf2md_list_profiles`는 option discovery라는 역할 구분을 문서화했다.
- `tests/test_mcp_server.py`가 OCR backend discovery contract를 검증한다.

### 검증

- `python3 -m pytest tests/test_mcp_server.py -q`

## P0 / Q127. Agent Skill Latest Sidecar Contract Sync

### 목표

기존 `pdf2md-rag-ingest` Skill을 새로 나누지 않고 Q121-Q125 최신 sidecar 계약과 맞춘다.

### 구현 결과

- Skill artifact map에 `page_layout_rag.jsonl`, `figure_ocr_evidence_rag.jsonl`, `figure_descriptions_rag.jsonl`, `figure_structures_rag.jsonl`를 추가했다.
- Skill validation/reporting guidance에 `adapter_metadata`, `cross_spec_compatibility`, `source_sha256`, `source_dedupe_key`, `stable_source_id`, `stable_requirement_seed` 보존 규칙을 추가했다.
- NVMe Base/NVM Command Set 보고 기준을 sidecar count, command relationship metadata coverage, Q125 metadata coverage, validator result 중심으로 정리했다.
- `docs/AGENT_SKILL_USAGE_GUIDE.md`와 `tests/test_agent_skill_pack.py`가 최신 sidecar/metadata guidance를 검증한다.
...
## P1 / Q126. GUI Batch Completion Summary Bounds

### 목표

GUI folder conversion에서 파일 수가 많은 경우 완료 화면이 지나치게 커져 결과 확인과 창 닫기가 어려워지는 문제를 줄인다.

### 구현 결과

- GUI batch completion summary를 bounded preview 중심으로 정리했다.
- 대량 batch 결과는 화면 전체 확장 대신 summary/count와 artifact 접근 흐름으로 확인하게 했다.

### 검증

- GUI batch completion UX regression test와 문서 동기화는 해당 구현 PR에서 수행했다.
...
- `pdf2md.serializers.rag_domain_adapters`에 `DomainAdapterSpec`, `DOMAIN_ADAPTER_REGISTRY_VERSION`, `supported_domain_adapter_specs()`, `get_domain_adapter_spec()`를 추가했다.
- registry metadata가 adapter별 SSD agent domain/spec type, revision hints, keyword profile, unit taxonomy, evaluator hooks, compatible adapters, required normalized fields를 보관한다.
- `domain_units_rag.jsonl` record에 `adapter_metadata`와 `cross_spec_compatibility`를 추가했다.
- NVMe command/status/field taxonomy와 OCP requirement taxonomy를 registry와 SSD-RAG validator 기준으로 분리했다.
- `manual` adapter는 core에서 비공개 schema를 고정하지 않고 SSD `HIL/CustomerRequirement`로 매핑한다.
- cross-spec stable source compatibility는 `source_sha256`, `source_dedupe_key`, `stable_source_id`, `stable_requirement_seed`를 기준 필드로 기록한다.
- `scripts/validate_ssd_rag_contract.py`가 registry 기반 adapter/spec mapping과 새 metadata를 검증한다. legacy domain unit record에는 `missing_adapter_metadata` warning만 남겨 backward compatibility를 유지한다.

### 제외 범위

- 외부 plugin package loading을 기본 활성화하지 않았다.
- customer-specific private schema를 core에 고정하지 않았다.
- downstream SSD verification agent 구현은 추가하지 않았다.

### 검증

- `tests/test_rag_domain_adapters.py`
- `tests/test_ssd_rag_contract.py`
- `tests/test_requirement_change_impact.py`
- latest NVMe/OCP smoke benchmark와 SSD-RAG contract validator
...
- `pdf2md.serializers.rag_figures`의 figure kind classifier를 `register_map`, `waveform`, `block_diagram`, `flow_diagram`, `table_like_image` 후보까지 확장했다.
- `figure_descriptions_rag.jsonl` record에 `semantics_schema_version`, `observed_text`, `generated_description`, `generated_content_scope`, `markdown_inserted`, `evidence_refs`, `review_required`, `review_reasons`, `hallucination_risk`를 추가했다.
- `figure_structures_rag.jsonl` record에 `structure_schema_version`, `observed_text`, `evidence_refs`, `relationship_hints`, review metadata를 추가했다.
- generated content와 observed content는 sidecar 안에서도 별도 field로 분리하고, `document.md` 삽입은 `markdown_inserted=false`로 고정했다.
- `scripts/validate_index_contract.py`와 `scripts/evaluate_figure_descriptions.py`가 Q124 review/content separation contract를 검증한다.
- report summary에 `figure_description_review_required_count`, `figure_structure_review_required_count`를 기록한다.

### 제외 범위

- generated description 기본 활성화는 하지 않았다.
- generated description을 Markdown 본문에 삽입하지 않았다.
- external VLM/API 호출이나 Docling runtime dependency를 추가하지 않았다.
- relationship hint는 검증된 edge가 아니라 review-only co-occurrence hint로 남겼다.
...
기본 `tesseract` OCR 경로를 유지하면서 optional OCR backend를 네이티브 registry/protocol로 확장한다.
optional dependency 미설치는 fatal이 아니라 structured warning/report로 처리한다.

### 구현 결과

- conversion-time OCR backend registry를 `tesseract`, `tesseract-cli`, `rapidocr`, `ocrmac`로 확장했다.
- `tesseract-cli` adapter는 `pytesseract` 없이 system `tesseract` executable을 사용한다.
- `rapidocr`, `ocrmac` adapter는 lazy import로 optional dependency를 유지한다.
- 새 adapter 파일은 `pdf2md/extractors/ocr_backends/tesseract_cli.py`,
  `pdf2md/extractors/ocr_backends/rapidocr.py`,
  `pdf2md/extractors/ocr_backends/ocrmac.py`에 둔다.
- `Config`, CLI `--ocr-backend`, MCP profile metadata는 새 backend 목록을 노출한다.
- OCR warning details에 `ocr_backend_raw_confidence_unit`,
  `ocr_backend_normalized_confidence_unit`, `ocr_backend_higher_is_better`,
  `ocr_backend_supports_languages` metadata를 추가했다.
- `scripts/probe_ocr_backends.py`의 `tesseract-cli` probe를 conversion path와 동일하게 pypdfium2 rendering dependency를 확인하도록 맞췄다.
- wheel contract에 새 OCR backend adapter files를 포함했다.

### 제외 범위

- 기본 backend는 변경하지 않았다. 기본값은 계속 `tesseract`다.
- optional dependency를 설치하거나 필수 dependency로 승격하지 않았다.
- OCR text를 자동 교정하거나 재서술하지 않았다.
- Docling OCR adapter는 conversion backend로 추가하지 않았다.

### 검증

- `tests/test_ocr.py`, `tests/test_config_and_io.py`, `tests/test_quality_gate_scripts.py`
- `tests/test_cli.py`, `tests/test_mcp_server.py`, `tests/test_docs_examples.py`

## P1 / Q122. Region OCR Evidence v2

### 목표

figure/table bbox crop OCR을 원문 대체가 아닌 evidence sidecar로 분리해, 이미지 업로드가 어려운 RAG 환경에서도 OCR evidence를 추적 가능하게 한다.
Markdown 본문, text block, table extraction 결과는 region OCR로 대체하지 않는다.

### 구현 결과

- `figure_region_ocr` full sidecar 경로에 `figure_ocr_evidence_rag.jsonl`을 추가했다.
- figure record는 기존 `figures_rag.jsonl.figure_region_ocr.region_ocr` 결과를 재사용해 중복 OCR을 피한다.
- table record는 row-wise `tables_rag.jsonl` source ref가 해소 가능한 table에 한해 bbox crop OCR evidence를 생성한다.
- evidence record는 `accepted`, `rejected`, `runtime_unavailable`, `not_attempted` status와 accepted/rejected reason taxonomy를 포함한다.
- 모든 evidence record는 `report_only=true`, `text_replaced=false`, `markdown_inserted=false`를 기록한다.
- report summary, manifest options, index/artifact/provenance validators, MCP merge registry, batch/corpus file map, benchmark sidecar 목록을 갱신했다.

### 제외 범위

- region OCR 결과를 Markdown, text block, table row, retrieval chunk source text로 승격하지 않았다.
- remote OCR/VLM 호출은 추가하지 않았다.
- row-wise table provenance가 없는 빈 table candidate OCR evidence는 이번 단계에서 제외했다.

### 검증

- `tests/test_rag_figures.py`, `tests/test_index_contract_validator.py`
- `tests/test_cli.py`, `tests/test_mcp_server.py`, `tests/test_output_schema_contract.py`, `tests/test_docs_examples.py`
- `tests/test_artifact_integrity_validator.py`, `tests/test_provenance_integrity_validator.py`

## P0 / Q121. Layout Sidecar and Reading Order Diagnostics

### 목표

기술 스펙 RAG 재처리를 위해 page-level reading order, column estimate, bbox region, caption linkage를 raw full text 없이 구조화한다.
기존 `document.md`, `text_blocks_rag.jsonl`, `retrieval_chunks_rag.jsonl` text source-of-truth 계약은 유지한다.

### 구현 결과

- `technical_spec_rag` 및 `technical_spec_rag_visual` full sidecar 경로에 `page_layout_rag.jsonl`을 추가했다.
- page record는 `layout_id`, page, source hash, reading-order strategy, column estimate, multi-column 여부, content/table/text/figure bbox summary를 포함한다.
- `region_refs[]`는 text/table/figure source id, role, bbox, order index만 담고 원문 region text는 포함하지 않는다.
- `caption_links[]`는 table/figure caption source id와 target id를 연결하며 caption text를 복제하지 않는다.
- `report.summary`에는 layout sidecar가 생성될 때만 layout count metric을 optional extras로 기록한다.
- index/artifact/provenance validator, MCP page-window merge registry, batch output file map을 새 sidecar에 맞춰 갱신했다.
- NVMe/OCP technical golden fixture에 `page_layout_rag.jsonl`을 추가했다.

### 제외 범위

- Docling layout 결과 병합은 추가하지 않았다.
- OCR/model layout 결과를 Markdown 또는 text block source-of-truth로 대체하지 않았다.
- raw full page text, raw region text, query/result text는 layout sidecar에 저장하지 않았다.

### 검증

- `tests/test_rag_layout.py`, `tests/test_index_contract_validator.py`
- `tests/test_cli.py`, `tests/test_mcp_server.py`, `tests/test_artifact_integrity_validator.py`, `tests/test_provenance_integrity_validator.py`, `tests/test_rag_tables.py`
- `tests/test_output_schema_contract.py`, `tests/test_docs_examples.py`, `tests/test_golden_corpus.py`
- latest NVMe Base front matter/table slice smoke 및 artifact/index/provenance validator

## P0 / Q120. Native Hybrid Chunking v2

### 목표

Docling식 document-native/hybrid chunking 아이디어를 `pdf2md`의 기존 RAG sidecar 계약 안에서 네이티브로 구현한다.
원문 chunk `text`는 변경하지 않고, section hierarchy, table context, final chunk id relationship metadata를 재처리 가능한 optional field로 강화한다.

### 구현 결과

- `retrieval_chunks_rag.jsonl` opt-in relationship metadata에 `relationship_metadata_version="2.0"`을 추가했다.
- merge/split 이후 최종 chunk id 기준으로 `chunk_group_index`, `chunk_group_count`, `section_chunk_index`, `section_chunk_count`를 기록한다.
- nested section chunk에는 `parent_section_path`, `parent_section_anchor_chunk_id`, `relationship_reasons`를 추가해 parent/section/neighbor 확장을 구분할 수 있게 했다.
- table-like chunk(`table_row`, `technical_table`, `domain_unit`)에는 source-derived `context_metadata`를 추가했다.
- table row chunk 생성 시 table-level caption/header/confidence를 row context로 상속해 repeated header context를 embedding 옵션과 독립적으로 보존한다.
- 기존 `relationship_strategy`, `previous_chunk_id`, `next_chunk_id`, `section_anchor_chunk_id`, `related_chunk_ids` 계약은 backward compatible하게 유지했다.

### 제외 범위

- 외부 vector DB, embedding service, LangChain/LlamaIndex output은 canonical artifact로 채택하지 않았다.
- generated summary를 chunk `text`로 삽입하지 않았다.
- table/figure extraction heuristic 대규모 변경은 Q121 이후 작업으로 남겼다.

### 검증

- `tests/test_rag_chunks.py`, `tests/test_rag_eval.py`, `tests/test_index_contract_validator.py`
- `tests/test_docs_examples.py`, `tests/test_golden_corpus.py`
...
기술 스펙 표를 더 안전하게 재처리하기 위해 table confidence, fallback reason, header/body/stub 구조,
continued table linkage를 구조화하고, technical table provenance에서 같은 confidence evidence를 사용할 수 있게 한다.

### 구현 결과

- extractor에서 `table_confidence_v2`, `table_confidence_v2_bucket`, `table_confidence_v2_reasons`를 계산한다.
- HTML fallback 표는 confidence가 `high`로 승격되지 않도록 최대 `medium`으로 cap한다.
- `TableAsset`, `report.summary.table_quality[]`, `tables_rag.jsonl` row record, `technical_tables_rag.jsonl`에 v2 confidence field를 전달한다.
- `report.summary.table_confidence_v2_buckets`와 `table_confidence_v2_average`를 추가해 table quality 분포를 집계한다.
- `docs/OUTPUT_SCHEMA.md`와 `docs/schema/manifest.schema.json`, `docs/schema/report.schema.json`을 갱신했다.
- table 관련 golden corpus를 새 confidence field 기준으로 갱신했다.

### 제외 범위

- 복잡 표를 GFM으로 강제 변환하지 않았다.
- table extraction backend, Docling runtime dependency, 모델 기반 table 승격은 추가하지 않았다.
- 기존 fallback reason과 low-quality advisory/actionable 정책은 유지했다.

### 검증

- `tests/test_tables.py`, `tests/test_rag_tables.py`, `tests/test_rag_technical_tables.py`
- `tests/test_docs_examples.py`, `tests/test_output_schema_contract.py`
- `tests/test_golden_corpus.py`
- latest NVMe Base table slice smoke 및 artifact/index/provenance validator

## P0 / Q118. Native Document IR and Serializer Boundary

### 목표

Docling의 `DoclingDocument` 설계에서 얻은 교훈을 `pdf2md` 네이티브 내부 표현으로 재구성한다.
외부 산출물 schema는 유지하면서 text/table/figure/page/source_ref를 하나의 내부 IR로 수렴하고,
Markdown serializer와 RAG sidecar 계층이 같은 source-of-truth에서 legacy record를 읽도록 한다.

### 구현 결과

- `pdf2md/document_ir.py`를 추가해 `Pdf2MdDocument`, `Pdf2MdPage`, `DocumentTextBlock`, `TableBlockRef`, `FigureBlockRef`, `SourceRef`, `PageLayoutSummary`를 정의했다.
- `build_pdf2md_document_ir` mapper가 기존 `PageResult`, text block, `TableAsset`, `ImageAsset`를 내부 IR로 수집한다.
- `ir_text_block_records`와 `ir_text_blocks_by_page` adapter가 기존 public `text_blocks_rag.jsonl` 및 Markdown serializer 입력 schema를 유지한다.
- `pdf2md/pipeline.py`에서 `rag_text_blocks` stage 직후 내부 IR을 생성하고, figure/semantic/retrieval chunk/Markdown serializer 입력을 IR 기반 legacy record로 일원화했다.
- public `document.md`, `manifest.json`, `report.json`, RAG sidecar schema는 변경하지 않았다.

### 제외 범위

- Docling JSON import, Docling runtime dependency, Docling Markdown canonical artifact 채택은 포함하지 않았다.
- table/figure extraction heuristic과 public JSON schema는 변경하지 않았다.
- 신규 public sidecar는 만들지 않았다.
...
- `.venv311/bin/python -m pytest tests/test_pipeline_smoke.py tests/test_pipeline_reporting.py tests/test_rag_tables.py tests/test_docs_examples.py`
- latest NVMe Base 1-3페이지 full sidecar smoke
- `scripts/validate_index_contract.py`, `scripts/validate_provenance_integrity.py`, `scripts/validate_artifact_integrity.py`
- `git diff --check`

## P0 / Q117. MCP NVMe Base Large Conversion Stability

### 목표

`ssd-verification-agent`가 pdf2md MCP를 통해 NVMe Base 700+ page급 PDF를 운영할 때 image/figure 병목,
장시간 실행 상태 불투명성, 중단 시 report 부재, 대형 단일 실행 리스크를 줄인다.

### 구현 결과

- 상세 구현 명세는 `docs/archive/PDF2MD_MCP_NVME_BASE_STABILITY_DEVELOPMENT_SPEC.md`로 보관한다.
- `image_mode=none`을 추가해 image box detection, image extraction, figure OCR/description/structure sidecar 생성을 명시적으로 skip한다.
- image extraction page/stage timeout과 figure semantics timeout을 structured warning/report counter와 progress event로 남긴다.
- MCP page-window workflow(`pdf2md_plan_page_windows`, `pdf2md_convert_page_window`, `pdf2md_merge_window_outputs`, `pdf2md_convert_pdf_windowed`)를 추가하고 sidecar merge 시 collision id를 deterministic하게 rewrite한다.
- `conversion_state.json`을 변환 중 유지하고, fatal exception/interrupt 상황에서 `interrupted_report.json` 및 best-effort `report.json` summary의 interrupted metadata를 남긴다.
- `docs/OUTPUT_SCHEMA.md`와 `docs/schema/`에 no-image, timeout, page-window merge, interrupted conversion 계약을 반영했다.

### 제외 범위

- `ssd-verification-agent` repo의 직접 ingest/API/MCP 구현은 이 repo에서 수행하지 않았다.
- external RAG server upload/indexing, visual VLM pixel interpretation, raw full text/image bytes report 저장은 범위에서 제외했다.

### 검증

- `tests/test_images.py`, `tests/test_pipeline_reporting.py`, `tests/test_pipeline_smoke.py`
- `tests/test_mcp_server.py`, `tests/test_rag_chunks.py`
- `tests/test_provenance_integrity_validator.py`, `tests/test_artifact_integrity_validator.py`
- `tests/test_docs_examples.py`, `tests/test_output_schema_contract.py`
- `git diff --check`

## P1 / Q116. SSD Verification Agent PDF2MD Sidecar Handoff

### 목표

`ssd-verification-agent`가 pdf2md 산출물을 local source of truth로 direct ingest하고, RAG server 검색 결과와 병행 운영할 수 있도록 repo 간 책임 경계와 handoff 계약을 확정한다.

### 구현 결과

- `docs/SSD_VERIFICATION_AGENT_PDF2MD_VISUAL_RAG_HANDOFF_SPEC.md`에 pdf2md sidecar bundle direct ingest 기준을 정리했다.
- ConvertPdfToMarkdown 책임을 PDF 변환, schema/validator, benchmark/report, visual sidecar 산출물 제공으로 고정했다.
- `ssd-verification-agent` 책임을 sidecar 등록/저장/ingest, local evidence DB, RAG result reconciliation, `SpecAnalysisAgent` scoring/API/MCP/workflow로 분리했다.
- API 후보 `POST /api/rag-documents/{rag_document_ref_id}/pdf2md-sidecars/ingest`와 MCP tool 후보 `ingest_pdf2md_sidecars(...)`의 request/response/count-only policy를 문서화했다.
- `figures_rag.jsonl`, `figure_descriptions_rag.jsonl`, `figure_structures_rag.jsonl`와 `figure_text`/`figure_description`/`figure_structure` retrieval chunk ingest 기준을 명시했다.
- `SpecAnalysisAgent` visual source quality scoring, visual candidate kind, NVMe/OCP visual category, RAG server/local sidecar reconciliation warning code를 handoff 기준으로 제공했다.

### 제외 범위

- `ssd-verification-agent` repo의 API/MCP/direct ingest/DB migration/agent scoring 구현은 이 repo에서 수행하지 않았다.
- PDF parsing/conversion logic을 `ssd-verification-agent`로 복제하지 않는다.
- public response/report에 raw spec text, table contents, image bytes, local PDF path를 노출하지 않는 정책을 유지한다.

### 검증

- handoff 문서가 Q115 visual sidecar contract와 같은 파일명, chunk type, provenance key를 사용하도록 대조했다.
- `tests/test_docs_examples.py`에서 active backlog 제거, archive 이동, handoff 문서 유지 계약을 검증한다.

## P1 / Q115. Visual Technical Spec RAG Profile and Metrics

### 목표

NVMe Base/Command와 OCP spec의 figure, diagram, waveform, register layout 검색성을 강화하기 위해 `technical_spec_rag_visual` profile과 visual sidecar/metric 계약을 공식화한다.
기존 `technical_spec_rag` 동작은 backward compatible하게 유지하고, visual semantics는 별도 profile 또는 명시 옵션으로만 켠다.

### 구현 결과

- `technical_spec_rag_visual`을 `SUPPORTED_RAG_PURPOSE_PROFILES`에 추가하고 CLI/Config/MCP/GUI/agent-pack workflow에 노출했다.
- Visual profile은 `technical_spec_rag` option matrix에 `rag_figure_text_chunks`, `figure_region_ocr`, `rag_generated_figure_descriptions`, `figure_structure_extraction`, `figure_description_backend=local-vlm`을 더한다.
- GUI에는 `기술 스펙 Visual RAG` preset과 `이미지 업로드 불가 RAG 대응` preset을 제공한다. assetless preset은 `technical_spec_rag_visual + image_mode=placeholder` 계약을 사용한다.
- `retrieval_chunks_rag.jsonl`의 `figure_text`, `figure_description`, `figure_structure` chunk source ref/provenance 검증을 `validate_index_contract.py`와 `validate_provenance_integrity.py` 기준으로 강화했다.
- `figure_descriptions_rag.jsonl`과 `figure_structures_rag.jsonl` public sidecar 계약을 `docs/OUTPUT_SCHEMA.md`와 schema contract에 반영했다.
- latest NVMe Base/Command/OCP benchmark wrapper에 `--visual-mode` option matrix, visual sidecar counters, aggregate-only `visual_eval` metric을 추가했다.
- `scripts/visual_rag_eval.py`를 추가해 raw query/result text 없이 `hit_at_k`, expected source coverage, figure source ref coverage를 평가한다.

### 제외 범위

- 외부 VLM/LLM 호출과 image pixel interpretation은 포함하지 않았다.
- generated figure description은 deterministic context summary이며 `document.md` 본문에 삽입하지 않는다.
- Visual evidence는 downstream agent에서 supporting evidence로 사용해야 하며, 규범 requirement 후보로 자동 승격하지 않는다.
...
## P1 / Q114. Docling Layout Adapter Comparison Mode

### 목표

Docling의 layout-aware table/figure/page representation을 현재 툴의 table/figure/RAG sidecar와 직접 병합하지 않고, sanitized metric으로만 비교한다.
Docling이 설치된 환경에서는 같은 PDF에 대해 count-only layout metric과 metric delta를 남기고, Docling 미설치 환경은 기존 advisory/gate 정책을 유지한다.

### 구현 결과

- `scripts/benchmark_docling_comparison.py`에 `layout_comparison_mode`와 CLI `--layout-comparison-mode off|summary`를 추가했다.
- `summary` mode에서는 current-tool report summary에서 `layout_table_candidate_count`, `layout_figure_candidate_count`, `layout_retrieval_chunk_count`를 기록한다.
- Docling run에서는 document dict key structure만 순회해 `layout_table_candidate_count`, `layout_figure_candidate_count`, `layout_page_candidate_count`, `layout_text_container_key_count`를 기록한다.
- `docling_artifact_comparison.json`의 `metric_deltas`가 current-tool/Docling layout count를 비교한다.
- `DoclingBenchmarkSummary`와 `DoclingArtifactSummary`에 layout comparison mode/enabled/comparable 필드를 추가하고 schema를 갱신했다.
- `scripts/run_latest_nvme_command_set_eval.py`와 `scripts/run_release_gates.py --gates docling`에 `--layout-comparison-mode` 전달 경로를 추가했다.

### 제외 범위

- Docling layout 결과를 `document.md`, table extraction, figure sidecar, retrieval chunks에 병합하지 않았다.
- raw Docling document dict, raw Markdown, PDF text, image bytes, customer path를 report에 저장하지 않았다.
- Docling을 필수 dependency로 추가하지 않았다.

### 검증

- `tests/test_quality_gate_scripts.py`에서 fake Docling converter로 sanitized layout count, metric delta, raw-content 비저장 계약을 검증한다.
- release gate command가 `--layout-comparison-mode summary`를 Docling-installed gate에 전달하는지 검증한다.
- schema export/check와 docs contract test로 `docling_benchmark_report`/`docling_artifact_comparison` summary 계약을 검증한다.

## P1 / Q113. Local-Only Figure Description Evaluation Pack

### 목표

Docling-informed picture description 후보를 실제 VLM/Docling backend에 연결하기 전에, 현재 `figure_descriptions_rag.jsonl` 산출물이 RAG 운영 원칙을 지키는지 deterministic하게 평가한다.
평가는 local-only JSON/JSONL 산출물만 읽고 raw image/PDF text/customer path를 저장하지 않는다.

### 구현 결과

- `FigureDescriptionEvalReport` 계열 모델과 `docs/schema/figure_description_eval_report.schema.json` public schema를 추가했다.
- `scripts/evaluate_figure_descriptions.py`를 추가해 변환 output directory의 `figure_descriptions_rag.jsonl`, `figures_rag.jsonl`, `retrieval_chunks_rag.jsonl`만 읽어 sanitized `figure_description_eval_report.json`을 생성한다.
- 정상 context-only description은 `generated_text=true`, `backend_status="not_invoked_context_only"`, source evidence/source refs, retrieval chunk link를 검증한다.
- finding은 `missing_figure_descriptions`, `missing_generated_text_flag`, `missing_source_refs`, `missing_source_evidence`, `visual_pixels_interpreted`, `backend_invoked`, `low_confidence`, `missing_retrieval_chunk`, `invalid_jsonl_record`를 기록한다.
- report summary는 figure/description/chunk count, evidence-backed count, low confidence, missing source ref/evidence, visual pixel interpretation, backend invocation, error/warning count를 제공한다.
- `scripts/run_release_gates.py`에 optional `figure-description-eval` gate를 추가해 release 전 output directory를 `--fail-on-error`로 검증할 수 있게 했다.

### 제외 범위

- VLM/Docling picture description 생성 실행은 포함하지 않았다.
- raw description body 외의 PDF 원문, image bytes, customer path는 report에 넣지 않았다.
- 기존 conversion output이나 `document.md` 본문은 수정하지 않았다.

### 검증

- `tests/test_quality_gate_scripts.py`에서 정상 sidecar pass, local-only 위반 finding, CLI `--fail-on-error`, release gate command wiring을 검증한다.
- `tests/test_output_schema_contract.py`에서 schema export order와 `purpose="local_figure_description_eval"` 계약을 검증한다.
- `tests/test_docs_examples.py`에서 README, output schema, Docling-informed design, active/archive 문서 정합성을 검증한다.
- 예시 release gate 명령: `.venv311/bin/python scripts/run_release_gates.py --output-dir /tmp/pdf2md-release-figure-description-eval --gates figure-description-eval --figure-description-eval-output-dir output`

## P1 / Q112. Region OCR Report-Only Prototype

### 목표

Q107에서 추가한 `figure_region_ocr` diagnostics를 기존 figure OCR 후보 승격에서 한 단계 확장해, figure bbox를 기준으로 페이지 영역을 렌더/crop하고 OCR을 시도하는 report-only prototype으로 만든다.
원문 Markdown, text blocks, table extraction 결과는 OCR region 결과로 대체하지 않는다.

### 구현 결과

- `augment_figure_records_with_region_ocr(...)`가 optional `pdf_path`, `ocr_lang`, `ocr_backend`을 받도록 확장했다.
- `--figure-region-ocr`가 켜진 pipeline에서는 figure bbox를 로컬에서 렌더/crop하고 Q111의 `tesseract` OCR adapter를 통해 region OCR을 시도한다.
- region OCR 결과는 `figures_rag.jsonl`의 `figure_region_ocr.region_ocr` diagnostics에만 기록한다.
- `region_ocr.report_only=true`, `region_ocr.text_replaced=false`를 기록해 기본 Markdown/text extraction과 명확히 분리했다.
- confidence threshold 이상 label만 `detected_labels`로 승격한다.
- runtime/crop/OCR 실패는 conversion warning이 아니라 `runtime_unavailable` 또는 `rejected` diagnostic으로 남긴다.
- report summary에 `figure_region_ocr_render_attempted_count`, `figure_region_ocr_region_candidate_count`, `figure_region_ocr_accepted_region_count`, `figure_region_ocr_rejected_region_count`, `figure_region_ocr_crop_rejected_count`, `figure_region_ocr_runtime_unavailable_count`를 추가했다.

### 제외 범위

- region OCR 결과를 `document.md` 본문이나 기본 text extraction으로 병합하지 않았다.
- table OCR, full visual description, Docling layout adapter는 포함하지 않았다.
- OCR backend는 Q111의 implemented conversion backend인 `tesseract`만 사용한다.

### 검증

- `tests/test_rag_figures.py`에서 existing candidate promotion 유지, region OCR accepted label promotion, runtime unavailable, invalid bbox rejection을 검증한다.
- 기존 golden corpus에서 기본 옵션 출력이 변하지 않음을 검증한다.

## P1 / Q111. OCR Backend Adapter Contract

### 목표

Q110 runtime probe에서 확인한 multi OCR backend 개념을 실제 변환 코드의 확장 지점으로 분리한다.
기본 OCR 품질과 결정성을 깨지 않기 위해 이번 작업에서는 `tesseract` backend만 변환 경로에 연결하고, optional backend는 runtime probe/report-only 근거 수준에 머문다.

### 구현 결과

- `pdf2md/extractors/ocr_backends/` package를 추가했다.
- backend protocol, normalized result, backend metadata, tesseract adapter, registry를 분리했다.
- 기존 full-page OCR 경로는 `tesseract` adapter를 통해 `pytesseract.image_to_string`과 `image_to_data`를 호출하도록 변경했다.
- `tesseract` adapter metadata는 raw confidence `0_to_100`, normalized confidence `0_to_1`, higher-is-better contract를 명시한다.
- `Config.ocr_backend`과 CLI `--ocr-backend`를 추가하되, 변환 경로에서 구현된 backend는 `tesseract`만 허용한다.
- GUI/batch path는 새 `ocr_backend` 기본값을 `Config`로 전달한다. GUI 선택 UI는 추가하지 않았다.
- `manifest.json` options와 OCR warning details에 `ocr_backend`을 기록한다.

### 제외 범위

- RapidOCR, EasyOCR, OCRmac, Docling OCR 실행 구현은 포함하지 않았다.
- OCR 결과를 기존 text/table/figure 결과와 자동 merge하거나 교정하지 않았다.
- OCR confidence threshold와 warning severity 판단은 변경하지 않았다.

### 검증

- `tests/test_ocr.py`에서 tesseract adapter 경유 fake pytesseract 호출, confidence warning, missing language data, unsupported backend warning을 검증한다.
- `tests/test_config_and_io.py`에서 unknown `ocr_backend` validation을 검증한다.
- CLI/GUI/pipeline/golden tests에서 `options.ocr_backend="tesseract"` 기록과 기존 Markdown/sidecar 동등성을 검증한다.

## P1 / Q110. Multi OCR Backend Runtime Probe

### 목표

Docling-informed 확장 후보 중 다중 OCR backend 도입을 바로 변환 경로에 연결하지 않고, 먼저 backend별 runtime readiness와 confidence normalization 정보를 deterministic report로 확인한다.

### 구현 결과

- `scripts/probe_ocr_backends.py`를 추가했다.
- probe 대상은 `tesseract`, `tesseract-cli`, `rapidocr`, `easyocr`, `ocrmac`, `docling`이다.
- probe는 OCR을 실제 PDF content에 적용하지 않고 module availability, executable path, Tesseract language data, platform support, confidence normalization hint만 기록한다.
- `ocr_backend_probe_report.json` public schema와 `OCRBackendProbeReport` 모델을 추가했다.
- `scripts/run_release_gates.py`에 optional `ocr-backends` gate를 추가했다.
- `--gates ocr-backends`는 기본적으로 하나 이상의 backend가 ready가 아니면 실패하며, `--ocr-backend-probe-no-require-ready`로 advisory probe처럼 사용할 수 있다.

### 제외 범위

- 기본 OCR 추출 backend는 변경하지 않았다.
- RapidOCR/EasyOCR/OCRmac/Docling OCR adapter를 변환 경로에 연결하지 않았다.
- backend별 OCR 결과 text를 비교하거나 후처리하지 않았다.

### 검증

- `tests/test_quality_gate_scripts.py`에서 ready/missing language probe, optional backend availability, release gate command wiring을 검증한다.
- `scripts/export_output_schema.py --check`로 `ocr_backend_probe_report.schema.json` 계약을 검증한다.

## P1 / Q109. Docling-Installed Benchmark Gate

### 목표

Q108에서 만든 latest NVMe benchmark evidence path를 Docling 설치 환경에서 release gate로 강제 실행할 수 있게 한다.
Docling이 설치되지 않은 일반 개발/CI 환경에서는 기존 advisory skip은 유지하되, `--require-docling` 또는 `--gates docling`을 명시한 경우에는 실패로 처리한다.

### 구현 결과

- `scripts/benchmark_docling_comparison.py`에 `require_docling` 옵션과 `--require-docling` CLI flag를 추가했다.
- `--require-docling` 상태에서 Docling이 미설치이면 기존 `docling_not_installed` warning에 더해 `docling_required_not_available` error finding을 기록한다.
- `scripts/run_latest_nvme_command_set_eval.py`에 `--require-docling`과 `--fail-on-error`를 추가했다.
- `scripts/run_release_gates.py`에 optional `docling` gate를 추가했다.
- `--gates docling`은 `--docling-input-pdf`를 필수로 요구하고, latest NVMe wrapper를 `--require-docling --fail-on-error --fail-on-current-tool-failure`로 실행한다.
- release gate report는 `docling_benchmark_report.json` 경로를 기록하고, wrapper는 기존 sanitized Docling comparison pack과 `latest_nvme_command_set_scorecard.md`를 생성한다.

### 제외 범위

- Docling을 필수 dependency로 추가하지 않았다.
- Docling 설치 자동화나 optional dependency download는 구현하지 않았다.
- Docling raw Markdown/dict export, PDF text, image bytes, customer path는 계속 저장하지 않는다.

### 검증

- `tests/test_quality_gate_scripts.py`에서 required Docling 미설치 error finding, docling gate input validation, docling gate command wiring을 검증한다.
- `.venv311/bin/python scripts/run_release_gates.py --output-dir /tmp/pdf2md-q109-docling-gate --gates docling --docling-input-pdf /tmp/NVM-Express-NVM-Command-Set-Specification-Revision-1.2-2025.08.01-Ratified.pdf --docling-pages 1`
- 현재 Docling 미설치 개발 환경에서는 위 gate가 의도대로 `exit_code=1`, `passed_release_gate=false`, `docling_required_not_available` error finding을 기록했다.
- 실제 Docling 미설치 환경에서 `--gates docling`은 의도적으로 실패한다. Docling 설치 환경에서만 release gate 통과를 기대한다.

## P1 / Q108. Latest NVMe Command Set Benchmark Evidence Path

### 목표

공식 최신 NVMe NVM Command Set specification을 로컬에서 반복 변환/비교할 수 있는 evidence 경로를 만든다.
기존 Docling comparison pack은 유지하되, Q107 figure semantics 옵션까지 current-tool benchmark에 전달해서 현재 코드 전체 기능 기준으로 평가한다.

### 구현 결과

- `scripts/benchmark_docling_comparison.py`의 current-tool 실행 경로에 `figure_region_ocr`, `rag_generated_figure_descriptions`, `figure_description_backend`, `figure_structure_extraction` 옵션을 추가했다.
- Docling comparison report metric에 actionable warning, HTML/GFM/low-quality table count, domain unit count, figure text/description/structure chunk count, region OCR diagnostics count를 추가했다.
- `scripts/run_latest_nvme_command_set_eval.py`를 추가했다.
- Q108 wrapper는 `technical_spec_rag + domain_adapter=nvme + image_mode=placeholder + rag_figure_text_chunks=true`를 기본으로 실행한다.
- 기본 `figure_semantics_mode=visual`에서는 Q107 deterministic context-only figure description/structure sidecar도 함께 평가한다.
- wrapper는 기존 `docling_benchmark_report.json`, `docling_artifact_comparison.json`, `docling_scorecard.md`를 그대로 생성하고, 사람이 검토할 `latest_nvme_command_set_scorecard.md`를 추가로 쓴다.
- scorecard는 raw PDF text, raw Markdown body, image bytes, customer path를 포함하지 않고 hash/status/metric만 기록한다.

### 제외 범위

- 공식 PDF binary를 저장소에 커밋하지 않는다.
- Docling을 필수 dependency로 추가하지 않는다.
- Docling raw Markdown/dict export나 이미지 bytes를 파일로 저장하지 않는다.
- wrapper의 advisory score는 local benchmark review 보조값이며 release gate threshold로 사용하지 않는다.

### 검증

- `tests/test_quality_gate_scripts.py`에서 Docling 미설치 상태의 sanitized pack, Q107 figure semantics option propagation, latest NVMe wrapper scorecard 생성을 검증한다.
- 공식 최신 PDF 실측은 repo 밖 `/tmp` 계열 경로에서 수행한다.
- `.venv311/bin/python scripts/run_latest_nvme_command_set_eval.py --input-pdf /tmp/NVM-Express-NVM-Command-Set-Specification-Revision-1.2-2025.08.01-Ratified.pdf --output-dir /tmp/pdf2md-q108-latest-nvme-command-set --fail-on-current-tool-failure`
- 최신 NVMe NVM Command Set Rev. 1.2 full conversion 결과는 `processed_pages=158`, `status=success`, `duration_ms=63357`, `pages_per_second=2.4956`, `actionable_warning_count=0`, `table_low_quality_count=0`, `retrieval_chunk_record_count=5972`, artifact/index/provenance validator pass로 확인했다.
- 현재 개발 환경에서는 Docling이 설치되어 있지 않아 `docling_not_installed` advisory skip이 기록됐고, Docling raw content는 저장하지 않았다.

## P1 / Q107. Assetless Figure Visual Semantics Layer

### 목표

이미지 파일 업로드가 불가능한 RAG 환경에서 회로도, 파형, 블록다이어그램처럼 이미지 자체 의미가 큰 figure의 검색성을 opt-in 방식으로 보강한다.
기존 `이미지 업로드 불가 RAG 대응` preset의 기본 보수 동작은 유지하고, 원문 Markdown과 생성/구조화 보강 정보를 분리한다.

### 구현 결과

- CLI/Config/Batch/GUI runner에 `figure_region_ocr`, `rag_generated_figure_descriptions`, `figure_description_backend`, `figure_structure_extraction` 옵션을 추가했다.
- GUI `Optimize Options(유저 선택)`에서 `Figure region OCR`, `Generated figure descriptions`, `Figure structure extraction`, `Figure description backend`를 설정할 수 있게 했다.
- 기존 assetless preset은 `placeholder + figure_text` 기본 동작을 유지하며, 생성형/구조화 옵션은 기본 off로 남겼다.
- `pdf2md.serializers.rag_figure_semantics`를 추가해 기존 figure provenance의 캡션, heading, 근접 텍스트, 기존 OCR 후보를 근거로 deterministic context-only 보강 record를 만든다.
- `figures_rag.jsonl`에는 opt-in `figure_region_ocr` diagnostics를 추가할 수 있다.
- `figure_descriptions_rag.jsonl`과 `figure_structures_rag.jsonl` sidecar를 추가했다.
- `retrieval_chunks_rag.jsonl`에 `chunk_type="figure_description"`과 `chunk_type="figure_structure"`를 추가했다.
- 생성형 설명 chunk는 `generated_text=true`, `generation_strategy`, `backend_status="not_invoked_context_only"`로 원문 텍스트와 분리한다.
- 구조화 chunk는 `derived_from_context=true`로 context-derived retrieval hint임을 표시한다.
- batch/corpus manifest file model, JSON schema, index/provenance/artifact validators, `docs/OUTPUT_SCHEMA.md`, GUI guide를 새 sidecar 계약에 맞춰 갱신했다.

### 제외 범위

- 실제 local VLM, Docling picture description, hosted API 호출은 dependency/비용/재현성 검토 전까지 연결하지 않았다.
- 이미지 binary/base64를 sidecar나 retrieval chunk에 저장하지 않았다.
- `document.md` 본문에는 생성형 figure description을 삽입하지 않았다.
- 저신뢰 구조에서 edge/component 추론은 하지 않고, deterministic evidence가 없는 경우 record를 생성하지 않는다.

### 검증

- `.venv311/bin/python -m compileall pdf2md scripts tests -q`
- `.venv311/bin/python -m pytest tests/test_rag_figures.py tests/test_cli.py::test_cli_accepts_rag_figure_text_chunks_option tests/test_gui_runner.py::test_gui_request_builds_single_config_from_cli_options tests/test_gui_runner.py::test_gui_batch_config_preserves_cli_option_contract tests/test_gui_profiles.py tests/test_gui_presets.py -q`
- `.venv311/bin/python -m pytest tests/test_output_schema_contract.py tests/test_docs_examples.py tests/test_index_contract_validator.py tests/test_artifact_integrity_validator.py tests/test_ssd_rag_contract.py -q`
- `.venv311/bin/python -m pytest -q`
- `.venv311/bin/python scripts/export_output_schema.py --check`
- `git diff --check`

## P1 / Q106. GUI Assetless RAG Preset And Manual Domain Adapter

### 목표

이미지 파일 업로드가 불가능한 팀 RAG 환경에서 GUI 사용자가 별도 옵션 조합을 기억하지 않고 기술 스펙 변환을 실행할 수 있게 한다.
또한 기존 NVMe/PCIe/OCP/TCG/SPDM/Caliptra/customer adapter에 맞지 않는 고객 requirement 표를 manual 입력으로 보수적으로 domain unit화한다.

### 구현 결과

- GUI preset에 `이미지 업로드 불가 RAG 대응`을 추가했다.
- 해당 preset은 내부적으로 `technical_spec_rag` profile을 사용하고 `image_mode=placeholder`, `rag_figure_text_chunks=true`를 적용한다.
- `DomainAdapterMode.MANUAL = "manual"`과 `manual_domain_adapter_label`, `manual_domain_adapter_keywords` 옵션을 CLI/GUI/batch/config/manifest/report 경로에 추가했다.
- manual adapter는 사용자 keyword를 table header matching에만 사용하고, domain unit `text`, `name`, `value`, `description`은 관측된 table/technical-table provenance에서만 복사한다.
- GUI profile import/export는 manual label/keywords를 option으로 저장하되 password, path, raw PDF/Markdown/table/image content는 계속 제외한다.
...
Q104 벤치마크 하네스와 Docling 공식 문서 기준으로 다중 OCR backend, region OCR, picture description, layout-aware table/figure adapter 도입 방향을 정한다.
원문 보존, deterministic output, partial success/report 원칙을 깨지 않는 opt-in/adapter 경계를 명확히 한다.

### 구현 결과

- `docs/DOCLING_INFORMED_EXTENSION_DESIGN.md`를 추가했다.
- Q104 local smoke에서 Docling이 설치되지 않아 `docling_not_installed` advisory skip만 확보됐음을 설계 판단의 제약으로 기록했다.
- Docling official reference point를 `DocumentConverter`, OCR engines, pipeline options, table extraction options, picture description, CLI image/table mode로 분리해 정리했다.
- current tool baseline은 Tesseract/pypdfium2 OCR, Q98-Q100 성능 개선, Q103 figure_text chunk, Q101 table adaptive/fallback 정책으로 정리했다.
- 후보별 decision matrix를 작성했다.
  - multi OCR backend: 설계만 유지, 구현 승격 보류
  - region OCR: report-only prototype 후보, 기본 text 대체 금지
  - picture description: 기본 변환/ingest 승격 금지, local-only 평가 pack 후보
  - layout adapter: comparison-only adapter 후보, current output과 merge/blend 금지
- 후속 구현이 필요할 때의 adapter boundary, public option 후보, manifest/report/warning schema 후보를 문서화했다.
- Docling-installed benchmark evidence가 확보되기 전에는 신규 implementation backlog를 만들지 않는다고 결정했다.

### 제외 범위

- OCR backend adapter, region OCR, picture description, Docling adapter를 실제 코드 경로에 추가하지 않았다.
- Docling을 dependency로 추가하지 않았다.
- remote VLM/API 호출 경로를 추가하지 않았다.
- Q106+ 신규 active backlog는 만들지 않았다.

### 검증

- `tests/test_docs_examples.py`에서 Q105 설계 문서, active backlog 없음, archive 이동 상태를 검증한다.
- 기존 Q104 하네스와 schema contract는 그대로 유지한다.

## P1 / Q104. Docling Benchmark Harness And Comparison Pack

### 목표

Docling의 OCR/layout/table/figure export 경로를 현재 툴과 같은 입력 PDF에서 비교할 수 있는 local-only harness를 제공한다.
감으로 기능을 도입하지 않고, Q105 확장 설계가 count/hash/metric/advisory finding을 근거로 판단되도록 한다.

### 구현 결과

- `scripts/benchmark_docling_comparison.py`를 추가했다.
- current-tool 변환은 `technical_spec_rag`, optional `--domain-adapter`, `--image-mode placeholder`, `--rag-figure-text-chunks` 경로로 실행할 수 있다.
- Docling이 설치된 환경에서는 `DocumentConverter().convert(...)` 결과에서 Markdown export와 document dict export를 in-memory로 수집한다.
- Docling 미설치 환경에서는 실패하지 않고 `docling_not_installed` advisory finding과 `status="skipped"` run을 기록한다.
- `docling_benchmark_report.json`은 current-tool/Docling status, duration, pages/sec, backend availability, validator metric, finding summary를 기록한다.
- `docling_artifact_comparison.json`은 current artifact와 Docling virtual artifact의 existence, size, SHA-256, numeric metric delta만 기록한다.
- `docling_scorecard.md`는 사람이 검토하기 위한 local scorecard를 생성한다.
- `DoclingBenchmarkReport`와 `DoclingArtifactComparisonReport` 모델 및 `docs/schema/` JSON Schema export 계약을 추가했다.
- `docs/RAG_INDEXER_INTEGRATION_RECIPES.md`에 RAG 운영자가 Q105 설계 근거로 사용할 comparison pack 생성 절차를 문서화했다.

### 제외 범위

- Docling을 필수 dependency로 추가하지 않았다.
- Docling raw Markdown body나 raw document dict를 파일로 저장하지 않는다.
- raw PDF, image bytes, 고객 식별 path는 report/comparison pack에 넣지 않는다.
- Docling adapter, OCR backend 교체, picture description 생성은 Q105 설계 범위로 남겼다.

### 검증

- `tests/test_quality_gate_scripts.py`에서 Docling 미설치 advisory pack 생성과 numeric metric delta 계산을 검증한다.
- `tests/test_docs_examples.py`와 `tests/test_output_schema_contract.py`에서 README/docs/schema 계약 동기화를 검증한다.
- `scripts/export_output_schema.py --check`로 `docling_benchmark_report.schema.json`과 `docling_artifact_comparison.schema.json`이 최신인지 확인한다.

## P0 / Q103. Assetless Technical RAG Figure Text Chunks

### 목표

PNG/JPG 같은 이미지 파일을 업로드할 수 없는 팀 RAG 환경에서도 기술 스펙 figure/diagram의 검색성과 provenance를 유지한다.
기본 이미지 추출 정책은 바꾸지 않고, opt-in `placeholder + figure_text chunk` 경로를 제공한다.

### 구현 결과

- `--rag-figure-text-chunks` CLI/config/batch/GUI/profile 옵션을 추가했다.
- `pdf2md.serializers.rag_chunks.build_retrieval_chunks()`가 opt-in `chunk_type="figure_text"` record를 생성한다.
- `figure_text.text`는 caption, heading path, detected labels, nearby text refs, conservative `figure_kind`만 사용한다.
- 생성형 picture description, VLM 설명, 추론 요약은 기본 출력에 넣지 않는다.
- `figure_text.source_refs[]`는 `figures_rag.jsonl`의 `figure_id`, page, bbox로 해소되고 image path는 넣지 않는다.
- `image_mode=placeholder|embedded`에서는 artifact integrity validator가 manifest/figure path를 provenance로 보고 missing image file 오류를 내지 않는다.
- `--rag-sidecar-scope minimal --rag-figure-text-chunks` 조합에서는 provenance 해소를 위해 `figures_rag.jsonl`도 함께 쓴다.
- manifest에는 opt-in 시 `options.rag_figure_text_chunks=true`를 기록하고, report summary에는 `rag_figure_text_chunks=true`, `figure_text_chunk_record_count`를 기록한다.
- README, `docs/OUTPUT_SCHEMA.md`, `docs/RAG_INDEXER_INTEGRATION_RECIPES.md`에 assetless RAG 운영 절차와 schema policy를 문서화했다.

### 제외 범위

- 기본 `image_mode=referenced`와 기존 full output contract는 변경하지 않았다.
- 이미지 파일을 강제로 삭제하거나 referenced mode를 자동 placeholder로 바꾸지 않았다.
- Docling/VLM 기반 picture description 도입 여부는 Q105 설계 대상으로 남겼다.

### 검증

- `tests/test_rag_chunks.py`에 figure_text chunk 생성/diagnostics-only skip 단위 테스트를 추가했다.
- `tests/test_rag_figures.py`에 placeholder assetless pipeline과 `validate_index_contract.py`, `validate_provenance_integrity.py`, `validate_artifact_integrity.py` 통합 검증을 추가했다.
- `tests/test_cli.py`, `tests/test_gui_runner.py`, `tests/test_gui_profiles.py`, `tests/test_gui_presets.py`, `tests/test_artifact_integrity_validator.py`에서 옵션 전달과 validator 정책을 검증한다.

## P1 / Q102. Fast Output Profile And Sidecar Scope

### 목표

대량 변환이나 1차 검토에서 core artifact 중심으로 빠르게 산출하는 opt-in profile을 제공한다.
기본 `full` output contract는 유지하고, sidecar 생략은 manifest/report에 재처리 가능한 scope 정보로 기록한다.

### 구현 결과

- `OutputProfile(full|fast)`와 `RagSidecarScope(full|minimal|none)`를 추가했다.
- 기본값은 기존과 동일한 `full`이며, `--output-profile fast`만 지정하면 effective sidecar scope를 `none`으로 계산한다.
- `--rag-sidecar-scope minimal`은 `text_blocks_rag.jsonl`, `retrieval_chunks_rag.jsonl`, 요청된 table sidecar만 쓴다.
- `--rag-sidecar-scope none`은 RAG sidecar build/write를 건너뛰고 `document.md`, `manifest.json`, `report.json` 중심으로 산출한다.
- non-full scope에서는 `manifest.options`와 `report.summary`에 `output_profile`, `rag_sidecar_scope`, `rag_sidecar_omitted_outputs`, `rag_sidecar_omitted_reason`을 기록한다.
- CLI, batch runner, GUI runner/profile export/import가 같은 option matrix를 사용한다.
- artifact integrity validator는 omitted sidecar scope를 인식하고, 생략된 sidecar 파일이 남아 있으면 stale artifact warning을 기록한다.
- `scripts/benchmark_conversion.py`에서 `--output-profile`, `--rag-sidecar-scope`를 받을 수 있게 했다.

### 제외 범위

- 기본 `full` golden/output contract는 변경하지 않았다.
- `technical_spec_rag`, `confidential_rag` 등 기존 RAG profile 기본값은 바꾸지 않았다.
- sidecar scope를 GUI visible control로 추가하는 UI 작업은 별도 UX 범위로 남겼다.

### 검증

- `.venv311/bin/python -m pytest tests/test_rag_tables.py tests/test_cli.py tests/test_gui_runner.py tests/test_gui_profiles.py tests/test_artifact_integrity_validator.py tests/test_docs_examples.py tests/test_output_schema_contract.py -q`
- `.venv311/bin/python -m pytest tests/test_golden_corpus.py -q`
- `.venv311/bin/python scripts/export_output_schema.py --check`
- `.venv311/bin/python -m ruff check .`
- benchmark evidence:
  - `/private/tmp/pdf2md-q102-full/benchmark_report.json`
  - `/private/tmp/pdf2md-q102-fast/benchmark_report.json`
  - synthetic 100p `page_workers=1`: total 2013ms -> 1811ms, `document.md` hash unchanged, sidecar record counts omitted in fast scope.

## P1 / Q101. Table Strategy Adaptive Mode

### 목표

표 추출에서 기본 전략 품질이 충분한 경우 fallback table strategy 실행을 생략한다.
복잡 표를 억지로 GFM으로 내보내지 않는 기존 안전 정책과 table quality diagnostics는 유지하면서 성능 병목을 줄인다.

### 구현 결과

- `pdf2md/extractors/tables.py`에서 default strategy를 먼저 실행하고, header confidence, empty cell ratio, placeholder header ratio, unresolved reason을 검사해 안전한 후보에서만 fallback strategy를 생략한다.
- fallback 생략 조건은 `TABLE_ADAPTIVE_*` 상수로 고정했다.
- 복잡 표, stub column, merged/multi-row header, ambiguous grid, sparse layout 등 known fallback reason이 있는 후보는 기존처럼 `lines_strict`, `mixed_lines_text`까지 실행한다.
- 실제 adaptive skip이 발생한 표에는 `summary.table_quality[]`에 `strategy_runs`, `adaptive_skipped_strategies`, `adaptive_skip_reason`을 기록한다.
- debug table candidate에는 page별 실행/skip strategy를 남겨 fallback 생략 여부를 추적할 수 있다.
- public table mode와 GFM/HTML fallback 결정 규칙은 변경하지 않았다.

### 제외 범위

- 기본 `table_mode` 동작이나 output profile 기본값은 변경하지 않았다.
- 복잡 표를 GFM으로 강제 변환하지 않았다.
- table extraction backend 추가나 Docling adapter 도입은 Q105 설계 범위로 유지했다.

### 검증

- `.venv311/bin/python -m pytest tests/test_tables.py tests/test_rag_tables.py -q`
- `.venv311/bin/python -m pytest tests/test_golden_corpus.py -q`
- simple/technical fixture에서 adaptive skip diagnostics가 golden report에 반영되는 것을 확인했다.
- complex/continued/repeated-template 표는 fallback strategy를 계속 실행해 HTML fallback 안전 정책을 유지한다.

## P1 / Q100. OCR Page Parallelization

### 목표

스캔 PDF 또는 `--force-ocr` 경로에서 target page OCR을 bounded worker로 병렬 처리한다.
OCR runtime unavailable, language data missing, low confidence warning의 의미를 유지하고, page text/metrics/warning merge 순서는 deterministic하게 유지한다.

### 구현 결과

- `run_ocr()` 내부에 page-level OCR helper와 chunk worker를 분리했다.
- `worker_count` 인자를 추가하되 기본값은 `1`로 유지해 직접 호출 기본 동작은 기존 순차 처리와 같다.
- pipeline은 기존 `page_workers`의 effective count를 OCR worker bound로 전달한다. 별도 public CLI 옵션은 추가하지 않았다.
- OCR chunk worker는 worker당 `pypdfium2.PdfDocument`를 한 번 열고 chunk 내 page를 순서대로 처리한다.
- 병렬 실행 결과는 page number 기준으로 정렬해 `page_texts`, `ocr_pages`, `metrics_by_page`, warnings 순서를 결정적으로 merge한다.
- Tesseract executable resolution, dependency unavailable, language-data missing, empty result, low/critical confidence warning payload는 기존 계약을 유지했다.

### 제외 범위

- 별도 `ocr_workers` CLI/config 옵션은 추가하지 않았다.
- OCR text 후처리, 교정, 요약은 하지 않았다.
- OCR runtime이 없는 환경에서도 deterministic하게 통과해야 하므로 runtime-dependent golden은 추가하지 않았다.

### 검증

- `.venv311/bin/python -m pytest tests/test_ocr.py -q`
- `.venv311/bin/python -m pytest tests/test_pipeline_reporting.py -q`
- mock OCR tests에서 sequential/parallel page text, metrics, warning merge equivalence를 확인했다.
- pipeline에서 `page_workers=2`일 때 `run_ocr(..., worker_count=2)`로 전달되는 것을 확인했다.

## P0 / Q99. Page Worker Chunked Parallelization

### 목표

`page_workers > 1` 경로가 page마다 PDF를 다시 열어 synthetic 100p에서 single worker보다 크게 느려지는 문제를 해결한다.
worker가 page chunk를 받아 PDF를 한 번 열고 여러 page의 text/read-order/table-candidate를 처리하게 하며, single-worker 산출물과 hash equivalence를 유지한다.

### 구현 결과

- backward-compatible한 `PageWorkerBatchInput`과 `extract_page_worker_batch()`를 추가했다.
- `run_page_workers()`가 page 단위 future 대신 worker 수에 맞춘 contiguous page chunk를 만들고, batch worker 하나가 PDF를 한 번만 열어 chunk 내 page를 순서대로 처리하도록 변경했다.
- 기존 `PageWorkerInput`과 `extract_page_worker()`는 유지해 단일 page 호출 호환성을 보존했다.
- batch 내부에서도 page별 `PageWorkerResult`, warning, failed 상태를 유지해 특정 page/table candidate 실패가 전체 batch 실패로 번지지 않도록 했다.
- `pdf_open_count`는 batch당 1회만 집계되도록 첫 page result에만 기록한다.
- pipeline/report/manifest public field 이름은 변경하지 않았다.

### 제외 범위

- `page_workers=1` 기본 경로는 기존 shared PDF context 방식을 유지했다.
- process 기반 병렬화나 PDF page cache 공유 구조는 도입하지 않았다.
- table extraction strategy 자체의 최적화는 Q101로 남겼다.

### 검증

- `.venv311/bin/python -m pytest tests/test_page_workers.py -q`
- `.venv311/bin/python -m pytest tests/test_performance_metadata.py -q`
- `python3 scripts/benchmark_conversion.py --output-dir /tmp/pdf2md-q99-current --page-counts 100 --page-workers 1,4`
- synthetic 100p benchmark에서 worker output hash equivalence가 통과했다.
- synthetic 100p `page_workers=4`: `total_duration_ms=3028`, `pdf_open_count=6`, `page_worker_effective_count=4`.

## P0 / Q98. Lazy Structure Marker OCR

### 목표

실제 기술 스펙 PDF에서 가장 큰 병목으로 확인된 structure marker 이미지 OCR을 lazy/context-first 방식으로 재설계한다.
기본값에서 OCR을 끄지 않고, child heading 또는 sibling sequence context로 안전하게 결정 가능한 marker는 OCR 호출 없이 복구하며 unresolved marker에만 기존 Tesseract OCR 후보 수집을 수행한다.

### 구현 결과

- `PendingStructureMarker` 수집 단계에서 Tesseract OCR을 즉시 실행하지 않고 image bytes, bbox, title, parent/child heading context만 보관하도록 변경했다.
- `_resolve_structure_markers()`에 context-only pass를 추가했다. 다음 child heading으로 현재 marker를 결정할 수 있는 경우와 앞뒤 strong recovery로 sibling sequence를 보간할 수 있는 경우 OCR 없이 `context_validated=True` recovery를 생성한다.
- context-only로 해결되지 않은 marker에만 기존 multi-scale/PSM OCR과 cache/early-stop 정책을 적용한다.
- parent context는 단독으로 번호를 생성하지 않고, unresolved OCR 후보 정규화와 기존 fallback 판정에서만 사용해 오복구 위험을 줄였다.
- context-only recovery는 기존 `STRUCTURE_MARKER_RECOVERED_CONTEXT_VALIDATED` reason과 `child_heading_context` 또는 `sibling_sequence_context` strategy를 사용해 report/manifest 계약을 깨지 않는다.
- 이미지 업로드 산출물과 figure extraction 기본 동작은 변경하지 않았다.

### 제외 범위

- structure marker OCR을 기본 비활성화하는 옵션은 추가하지 않았다.
- parent heading만으로 suffix를 새로 만드는 복구는 오복구 위험이 있어 도입하지 않았다.
- 실제 NVMe PDF 원본은 커밋하지 않고, local-only benchmark evidence 경로만 검증 기록으로 남겼다.

### 검증

- `.venv311/bin/python -m pytest tests/test_images.py -q`
- `.venv311/bin/python -m pytest tests/test_pipeline_reporting.py tests/test_pipeline_smoke.py tests/test_golden_corpus.py -q`
- `python3 scripts/benchmark_conversion.py --output-dir /tmp/pdf2md-q98-baseline --page-counts 100 --page-workers 1`
- `python3 scripts/benchmark_conversion.py --output-dir /tmp/pdf2md-q98-current --page-counts 100 --page-workers 1 --baseline-report /tmp/pdf2md-q98-baseline/benchmark_report.json`
- synthetic 100p benchmark는 structure marker OCR을 포함하지 않지만, `document.md`, `tables_rag.jsonl`, `technical_tables_rag.jsonl` hash equivalence와 performance gate 통과를 확인했다.

## P2 / Q97. Modern Python Tooling And Packaging Readiness

### 목표

Python 3.11+ 프로젝트 표준에 맞춰 lint/type/package/audit 기반을 보수적으로 도입하되, core conversion behavior 변경과 분리한다.

### 구현 결과

- `pyproject.toml`에 ruff dev dependency와 최소 lint smoke 설정을 추가했다. 첫 단계에서는 syntax/undefined-name 계열만 CI에서 검사해 대규모 formatting churn을 피한다.
- `pdf2md/py.typed` marker를 추가하고 setuptools package data와 wheel contract inspector에서 typed package 포함 여부를 검증한다.
- GitHub Actions CI와 `ci-lightweight` release gate에 `python -m ruff check .` lint smoke를 추가했다.
- `dependency-audit` optional release gate를 추가했다. `pip-audit` 결과는 `dependency_audit_report.json` 경로로 남기되 첫 단계에서는 advisory record로 처리해 release 실패를 강제하지 않는다.
- `CHANGELOG.md`를 추가해 PR 중심 이력과 상세 quality archive 사이의 공개 변경 요약 위치를 마련했다.
- README의 tooling/package smoke 예시와 backlog 상태를 갱신하고 Q97을 active backlog/development specs에서 archive로 이동했다.

### 제외 범위

- 전체 코드 formatting 변경은 하지 않았다.
- mypy/pyright strict mode와 dependency major upgrade는 별도 신규 Q가 생길 때 다룬다.
- LICENSE는 프로젝트 소유자가 라이선스를 확정해야 하므로 임의로 추가하지 않았다.

### 검증

- `.venv311/bin/python -m ruff check .`
- `.venv311/bin/python -m pytest tests/test_quality_gate_scripts.py tests/test_docs_examples.py -q`
- `.venv311/bin/python scripts/run_release_gates.py --output-dir /tmp/pdf2md-q97-tooling-gates2 --gates ci-lightweight,dependency-audit`
- `.venv311/bin/python scripts/run_release_gates.py --output-dir /tmp/pdf2md-q97-tooling-gates-network --gates dependency-audit`
- `.venv311/bin/python -m build --wheel --no-isolation --outdir /tmp/pdf2md-q97-build`
- `.venv311/bin/python -m pip wheel . --no-deps --no-build-isolation -w /tmp/pdf2md-q97-dist`
- `.venv311/bin/python scripts/inspect_wheel_contract.py --dist-dir /tmp/pdf2md-q97-dist --report-file /tmp/pdf2md-q97-dist/wheel_contract_report.json`
- `git diff --check`
...
Storage/security spec corpus에서 NVMe/PCIe/OCP/TCG/SPDM/Caliptra 계열 table shape를 더 명확히 domain unit으로 분리하고, SPDM과 Caliptra를 first-class adapter로 지원한다.
