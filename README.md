...
scripts/
  ...
  probe_ocr_backends.py
  evaluate_figure_descriptions.py
...
완료 후에는 문서별 status, warning count/code, Markdown/report/manifest 경로를 확인할 수 있고, 선택한 결과 행의 `Open Markdown`, `Open Report`, `Open Manifest`, `Open Assets`, `Open output folder`로 산출물을 바로 열 수 있습니다. 폴더 배치 산출물은 `Open Corpus Manifest`, `Open Corpus Diff`, `Open Requirement Impact`로 열 수 있습니다.
...
GUI는 최근 입력 파일/폴더, output folder, 선택 언어, 선택 preset을 local-only state로 저장하지만, previous corpus manifest path, 원문 텍스트, 표, 이미지 내용, warning message는 저장하지 않습니다. 필요하면 `Clear recent`로 최근 경로를 지울 수 있습니다.
...
비개발자 배포는 현재 source/ZIP + venv setup + `python -m pdf2md.gui` 경로를 기본으로 권장합니다. PyInstaller/native bundle은 Tkinter, PyMuPDF, Tesseract, code signing/notarization smoke가 정리되기 전까지 공식 기본 배포 경로로 보지 않습니다.
...
이 runner는 Tk 창을 띄우지 않고 GUI runtime doctor diagnostics, `python -m pdf2md.gui --help`, preset별 single/batch runner smoke, isolated GUI state round-trip, 수동 GUI checklist 상태를 `gui_smoke_evidence.json`에 기록합니다.
...
Machine-readable schema는 `docs/schema/`에 있으며, 예를 들어 `docs/schema/manifest.schema.json`, `docs/schema/figure_description_eval_report.schema.json`, `docs/schema/local_corpus_evidence_pack.schema.json`, `docs/schema/corpus_evidence_analysis_report.schema.json`, `docs/schema/corpus_evidence_trend_report.schema.json`을 `python scripts/export_output_schema.py --check`로 검증합니다.
...
Claude Code, Cline, Roo Code, Cursor, Continue 같은 AI coding agent에서 재사용할 수 있는 공통 skill/rule 패키지는
[docs/AGENT_SKILL_PORTABILITY.md](/Users/mankiw/VS_Project/ConvertPdfToMarkdown/docs/AGENT_SKILL_PORTABILITY.md)에 정리합니다.
canonical Agent Skill은 `agent-pack/skills/pdf2md-rag-ingest/`에 두며, 도구별 설치는 아래처럼 생성합니다.
...
python3 -m pdf2md input.pdf -o output/ --rag-profile technical_spec_rag --domain-adapter nvme
python3 -m pdf2md input.pdf -o output/ --rag-profile confidential_rag
python3 -m pdf2md input.pdf -o output/ --rag-profile preserve_with_sidecars
```
...
./.venv311/bin/python scripts/benchmark_docling_comparison.py --input-pdf pdf/local-spec.pdf --output-dir /tmp/pdf2md-docling-comparison --document-label doc-0001 --layout-comparison-mode summary
./.venv311/bin/python scripts/run_latest_nvme_command_set_eval.py --input-pdf /tmp/NVM-Express-NVM-Command-Set-Specification-Revision-1.2-2025.08.01-Ratified.pdf --output-dir /tmp/pdf2md-latest-nvme-command-set --layout-comparison-mode summary
./.venv311/bin/python scripts/evaluate_figure_descriptions.py --output-dir output --fail-on-error
...
./.venv311/bin/python scripts/run_release_gates.py --output-dir /tmp/pdf2md-release-ocr-backends --gates ocr-backends --ocr-backend-probe-backends all
./.venv311/bin/python scripts/run_release_gates.py --output-dir /tmp/pdf2md-release-figure-description-eval --gates figure-description-eval --figure-description-eval-output-dir output
...
- `docling_benchmark_report.json`: 현재 툴과 Docling 실행 상태, duration/pages/sec, backend availability, validator metric, optional layout comparison metric, advisory finding을 raw text/image/customer path 없이 기록
- `docling_artifact_comparison.json`: current artifact와 Docling virtual export를 existence/size/SHA-256/metric delta로만 비교하며, layout comparison mode에서는 table/figure/page count delta를 추가 기록
...
- `ocr_backend_probe_report.json`: optional OCR backend별 module/executable/language readiness와 confidence normalization hint
- `figure_description_eval_report.json`: `figure_descriptions_rag.jsonl`이 local-only, source evidence/source refs, backend non-invocation, retrieval chunk linkage 원칙을 지키는지 raw image/PDF text/customer path 없이 평가
...
- `release_gate_report.json`: OCR preflight, multi OCR backend probe, corpus quality gate, benchmark performance gate, lightweight CI gate, advisory dependency audit gate, optional RAG calibration gate, optional Docling-installed benchmark gate, optional figure description eval gate, optional GUI headless smoke/support redaction gate, optional GUI/CLI parity gate, optional GUI/CLI benchmark gate, schema check, packaging smoke/wheel contract command/status summary
...
- GitHub Actions CI는 PR/push마다 schema contract, docs/output contract, `python -m ruff check .`, `python -m pytest`, `python -m pdf2md --help`를 실행합니다.
...
lint / format / packaging tooling 예시:

```bash
./.venv311/bin/python -m ruff check .
./.venv311/bin/python -m ruff format --check .
./.venv311/bin/python -m pip wheel . --no-deps --no-build-isolation -w /tmp/pdf2md-dist
./.venv311/bin/python scripts/inspect_wheel_contract.py --dist-dir /tmp/pdf2md-dist --report-file /tmp/pdf2md-dist/wheel_contract_report.json
```

`pdf2md/py.typed`는 wheel에 포함되어야 하며, dependency audit은 `--gates dependency-audit`에서 advisory record로 먼저 관측합니다.
...
- 완료된 Q34-Q114 품질 개선 명세와 구현 결과는 `docs/QUALITY_IMPROVEMENT_IMPLEMENTED_SPECS.md`에서 확인합니다.
