...
- Add Q158 local Unicode/Hangul BM25 evaluation and verified Windows/Linux CPU CI. / Q158 한·영 BM25 평가 및 Windows/Linux CI 검증을 완료했다. 토큰 부분 문자열 오판과 Windows GUI smoke 경로 오판을 수정했다. 질문 정답의 사용자 승인은 대기 중이다.
...
- Add Q148 conversion journal artifact inventory throttling so running progress state avoids repeated full output directory scans while terminal state keeps deterministic artifact inventory.
- Add Q141-Q142 domain recommendation hardening: raw-content-free recommendation basis, ambiguity guard, and customer requirement coverage.
