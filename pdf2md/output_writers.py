...
from pdf2md.utils.io import write_json, write_jsonl, write_text
...
def write_rag_table_outputs(
    *,
    ...
    rag_table_records = flatten_rag_table_records(rag_tables)
    record_count = len(rag_table_records)
    file_count = 0
    ...
    if output_mode.writes_jsonl():
        write_jsonl(config.output_dir / config.rag_tables_jsonl_filename, rag_table_records)
        file_count += 1
    return record_count, file_count


def write_rag_text_block_output(config: Config, records: list[dict]) -> tuple[int, int]:
    write_jsonl(config.output_dir / config.rag_text_blocks_jsonl_filename, records)
...
def write_semantic_layer_outputs(
    ...
    *,
    ...
    write_jsonl(config.output_dir / config.semantic_units_jsonl_filename, semantic_units)
    write_jsonl(config.output_dir / config.requirements_jsonl_filename, requirements)
    write_jsonl(config.output_dir / config.cross_refs_jsonl_filename, cross_refs)
...
def write_retrieval_chunk_output(config: Config, records: list[dict]) -> tuple[int, int]:
    write_jsonl(config.output_dir / config.retrieval_chunks_jsonl_filename, records)
    return len(records), 1


def write_page_layout_output(config: Config, records: list[dict]) -> tuple[int, int]:
    write_jsonl(config.output_dir / config.page_layout_jsonl_filename, records)
    return len(records), 1


def write_figure_rag_output(config: Config, records: list[dict]) -> tuple[int, int]:
    write_jsonl(config.output_dir / config.figures_rag_jsonl_filename, records)
    return len(records), 1


def write_figure_ocr_evidence_output(config: Config, records: list[dict]) -> tuple[int, int]:
    write_jsonl(config.output_dir / config.figure_ocr_evidence_jsonl_filename, records)
    return len(records), 1


def write_figure_description_output(config: Config, records: list[dict]) -> tuple[int, int]:
    write_jsonl(config.output_dir / config.figure_descriptions_jsonl_filename, records)
    return len(records), 1


def write_figure_structure_output(config: Config, records: list[dict]) -> tuple[int, int]:
    write_jsonl(config.output_dir / config.figure_structures_jsonl_filename, records)
    return len(records), 1


def write_domain_unit_output(config: Config, records: list[dict]) -> tuple[int, int]:
    write_jsonl(config.output_dir / config.domain_units_jsonl_filename, records)
    return len(records), 1


def write_requirement_traceability_output(config: Config, records: list[dict]) -> tuple[int, int]:
    write_jsonl(config.output_dir / config.requirement_traceability_jsonl_filename, records)
    return len(records), 1


def write_technical_table_output(config: Config, records: list[dict]) -> tuple[int, int]:
    write_jsonl(config.output_dir / config.technical_tables_jsonl_filename, records)
    return len(records), 1
